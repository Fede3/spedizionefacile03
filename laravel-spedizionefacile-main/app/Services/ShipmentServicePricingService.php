...
class ShipmentServicePricingService
{
    ...
    public function getPricingConfig(): array
    {
        ...
        $normalized = $this->normalizeConfig(is_array($decoded) ? $decoded : self::DEFAULT_CONFIG);
    ...
    {
        $defaults = self::DEFAULT_CONFIG['service_pricing'];
        $normalized = [];
    ...
    {
        $defaults = self::DEFAULT_CONFIG['automatic_supplements'];
        $normalized = [];
    ...
    {
        $defaults = self::DEFAULT_CONFIG['operational_fees'];
        $normalized = [];
