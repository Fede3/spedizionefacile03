...
/**
 * FILE: OrderExportController.php
 * SCOPO: Esporta gli ordini dell'utente autenticato in formato CSV.
 *
 * ENDPOINT:
 *   - GET /api/orders/export (auth:sanctum + throttle 5/min)
 *
 * PARAMETRI QUERY:
 *   - from     (YYYY-MM-DD, opzionale): data inizio intervallo (>= created_at)
 *   - to       (YYYY-MM-DD, opzionale): data fine intervallo (<= created_at)
 *   - status   (string, opzionale): stato ordine (vedi costanti Order::*)
 *
 * OUTPUT:
 *   CSV con header: order_id, data, stato, mittente, destinatario, peso,
 *                   servizio, totale_eur, tracking_brt
 *   Prezzi in euro formato italiano (virgola come separatore decimale).
 *   Max 1000 righe per export (MAX_ROWS).
 *
 * SICUREZZA:
 *   - Solo ordini dell'utente autenticato (o tutti se admin).
 *   - StreamedResponse per evitare di caricare tutto in memoria.
 */
