...
/**
 * FILE: Admin/AdminBankTransferController.php
 * SCOPO: Admin conferma la ricezione di un bonifico in attesa (F05 — audit BRT 2026-04-18).
 *
 * FLUSSO:
 *   1. Cliente sceglie "Bonifico" nel checkout → ordine in status awaiting_bank_transfer.
 *   2. Cliente riceve email con IBAN + causale = "Ordine #{id}".
 *   3. Admin controlla l'estratto conto banca e conferma tramite:
 *        POST /api/admin/orders/{order}/confirm-bank-transfer
 *      Si crea una Transaction type=bonifico status=succeeded e l'ordine passa a
 *      COMPLETED, scatenando OrderPaid → generazione etichetta BRT standard.
 *
 * ENDPOINT ADDIZIONALE:
 *   GET /api/admin/orders/awaiting-bank-transfer — lista filtrata dei bonifici pendenti.
 *
 * SICUREZZA:
 *   - auth:sanctum + CheckAdmin applicati in routes/api/admin.php
 *   - L'ordine deve essere effettivamente in status awaiting_bank_transfer.
 */
