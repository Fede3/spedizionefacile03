...
use App\Http\Requests\StripePaymentMethodRequest;
...
class StripeCustomerController extends Controller
{
    ...
    public function setDefaultPaymentMethod(StripePaymentMethodRequest $request)
    {
        $user = $request->user();
        if (! $user->customer_id) {
    ...
    public function changeDefaultPaymentMethod(StripePaymentMethodRequest $request)
    {
        $user = $request->user();
        if (! $user->customer_id) {
            return response()->json(['error' => 'No Stripe customer'], 400);
    ...
    public function deleteCard(StripePaymentMethodRequest $request)
    {
        $user = $request->user();
        if (! $user->customer_id) {
