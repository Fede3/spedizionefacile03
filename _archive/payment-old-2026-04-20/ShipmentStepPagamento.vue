...
// Guard: pagamento "vuoto" quando total === 0 e non siamo in success.
// Evita lo stato "ghost": riepilogo 0,00 € con nessun metodo pagamento visibile,
// tipico arrivo diretto a /la-tua-spedizione/2?step=pagamento senza order_id
// valido (es. refresh dopo checkout_success, navigazione manuale, cart stale).
const hasValidAmount = computed(() => {
	const raw = String(props.finalTotalFormatted || props.summaryTotalPrice || '').replace(/[^0-9,]/g, '');
	const num = Number(raw.replace(',', '.'));
	return Number.isFinite(num) && num > 0;
});
const showEmptyState = computed(() => props.isOpen && !props.paymentSuccess && !hasValidAmount.value);
</script>
		...
		<button
			...
			<div class="shipment-stage-card__copy">
				...
				<p v-if="!isOpen" class="shipment-stage-card__summary">
					{{ paymentSuccess ? 'Pagamento completato' : `${trattaLabel} · ${finalTotalFormatted || summaryTotalPrice || 'Totale da definire'}` }}
				</p>
			</div>
				...
				<div class="shipment-stage-card__body-inner">
					...
					<div class="payment-summary-card">
						...
						<div class="flex flex-col gap-[10px] lg:flex-row lg:items-start lg:justify-between">
							...
							<div class="flex items-center gap-[10px] lg:justify-end">
								...
								<p
									...
									{{ finalTotalFormatted || summaryTotalPrice || 'Da definire' }}
								</p>
							</div>
						</div>
						...
						<button
							...
							<span>{{ paymentSummaryExpanded ? 'Nascondi dettagli ordine' : 'Vedi dettagli ordine e modifica' }}</span>
							<span
										...
										<div class="min-w-0">
											...
											<div class="payment-summary-section__route">
												<div class="payment-summary-section__route-item">
													...
													<span class="payment-summary-section__title">{{ confirmationOriginContact }}</span>
													<span class="payment-summary-section__body">
														{{ originAddress.address || 'Indirizzo da completare' }} {{ originAddress.address_number || '' }}
													</span>
													<span class="payment-summary-section__body">
														{{ originAddress.postal_code || 'CAP' }} {{ originAddress.city || '' }} {{ originAddress.province || '' }}
													</span>
												</div>
												<div class="payment-summary-section__route-item">
													<span class="payment-summary-section__route-label">{{ deliveryMode === 'pudo' ? 'Destinazione (Punto BRT)' : 'Destinazione' }}</span>
													<span class="payment-summary-section__title">{{ confirmationDestinationContact }}</span>
													<span class="payment-summary-section__body">
														{{ destinationAddress.address || (deliveryMode === 'pudo' ? 'Consegna presso punto BRT' : 'Indirizzo da completare') }} {{ destinationAddress.address_number || '' }}
													</span>
													<span class="payment-summary-section__body">
														{{ destinationAddress.postal_code || 'CAP' }} {{ destinationAddress.city || '' }} {{ destinationAddress.province || '' }}
													</span>
												</div>
											</div>
										</div>
								...
								<article class="payment-summary-section">
									<header class="payment-summary-section__header">
										<div class="min-w-0">
											...
											<p class="payment-summary-section__title">{{ paymentSummaryServicesLabel }}</p>
