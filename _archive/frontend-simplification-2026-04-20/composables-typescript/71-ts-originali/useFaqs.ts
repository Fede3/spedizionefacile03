/**
 * useFaqs — Composable dedicato alla pagina FAQ.
 *
 * Espone:
 *  - `categories`: array di categorie ufficiali (ordine determinato).
 *  - `faqs`: dataset statico minimo 25 FAQ italiane reali, ognuna con
 *    `id`, `category`, `question`, `answer`.
 *  - `escapeHtml` / `highlightMatch`: helper per evidenziare i match
 *    di ricerca senza introdurre dipendenze esterne (no Fuse.js).
 *
 * Tutto è puro / serializzabile: nessuna chiamata API, nessuna
 * reattività interna. La pagina costruisce ref/computed sopra
 * questo dataset.
 */
...
/**
 * Evidenzia (con tag <mark>) le occorrenze case-insensitive
 * della query dentro al testo. La query viene escapata per
 * non rompere la regex.
 */
...
export function useFaqs() {
	...
	return {
		faqs: FAQS as readonly FaqItem[],
		categories: FAQ_CATEGORIES as readonly FaqCategory[],
		escapeHtml,
		highlightMatch,
	};
}
