...
export const useAdminPrezziList = ({
	...
	const europeBandFilters = computed(() => [
		...
		...(Array.isArray(europePricing.value?.bands) ? europePricing.value.bands : []).map((band) => ({ value: band.id, label: band.label })),
	]);
	...
	const filteredEuropeBands = computed(() => {
		...
		// Difesa SSR: durante l'idratazione europePricing.bands puo' essere undefined
		// prima che fetchPriceBands() risolva. Stesso discorso per band.rates vuoto.
		const bands = Array.isArray(europePricing.value?.bands) ? europePricing.value.bands : [];

		return bands
			.filter((band) => selectedBand === 'all' || band.id === selectedBand)
			.map((band) => {
				const bandRates = Array.isArray(band?.rates) ? band.rates : [];
				const rates = sortRates(bandRates.filter((rate) => {
