...
interface QuickQuotePackage {
	...
	europe_quote?: unknown
	[key: string]: unknown
}

interface PriceBandsRef {
	europe?: {
		enabled?: boolean
...
interface QuickQuoteShipmentFlowStore {
...
interface UseQuickQuotePackagesDeps {
	...
	getCapSupplement: (originCap: string, destCap: string) => number
	getEuropeQuote: (country: string, weight: number, volume: number) => { status?: string; price?: number } | null
	priceBands: Ref<PriceBandsRef>
}

const QUICK_QUOTE_PACKAGE_TYPES: PackageTypeConfig[] = [
...
const syncQuickQuotePackageCounter = (packages: QuickQuotePackage[] = []): void => {
...
const findPackageTypeConfig = (packageType: unknown): PackageTypeConfig => {
	const normalized = normalizePackageType(
		typeof packageType === 'string' ? packageType : (packageType as { text?: string })?.text,
	)
	const found = QUICK_QUOTE_PACKAGE_TYPES.find(
		(item) => normalizePackageType(item.text) === normalized,
	)
	return found || (QUICK_QUOTE_PACKAGE_TYPES[0] as PackageTypeConfig)
}

const ensurePackageDraftIdentity = (pack: QuickQuotePackage = {}, fallbackType: unknown = null): QuickQuotePackage => {
...
const buildPackageDraft = (packageType: unknown): QuickQuotePackage => {
...
export const useQuickQuotePackages = ({
	...
	getEuropeQuote,
	priceBands,
}: UseQuickQuotePackagesDeps) => {
	const ensurePackagesIdentity = (): void => {
	...
	const isEuropeMonocollo: ComputedRef<boolean> = computed(() => {
	...
	const enforceEuropeMonocollo = (): void => {
	...
	const getPackVisual = (pack: QuickQuotePackage): { img: string; width: number; height: number } => {
		const fallback = QUICK_QUOTE_PACKAGE_TYPES[0] as PackageTypeConfig
	...
	const recalculatePackagesTotal = (): void => {
	...
	const calcQuantity = (pack: QuickQuotePackage): void => {
	...
	const checkPrices = (pack: QuickQuotePackage): void => {
		if (isEuropeMonocollo.value) {
			...
			if (!Number.isFinite(weight) || weight <= 0 || !Number.isFinite(firstSize) || firstSize <= 0 || !Number.isFinite(secondSize) || secondSize <= 0 || !Number.isFinite(thirdSize) || thirdSize <= 0) {
				pack.weight_price = null; pack.volume_price = null; pack.single_price = null; pack.single_priceOrig = null
				return
			}
			...
			if (quote?.status === 'priced') {
				...
				pack.quantity = 1; pack.weight_price = price; pack.volume_price = price; pack.single_price = price; pack.single_priceOrig = price
				recalculatePackagesTotal()
				return
			}
			pack.weight_price = null; pack.volume_price = null; pack.single_price = null; pack.single_priceOrig = null
	...
	const calcPriceWithWeight = (pack: QuickQuotePackage): void => {
		if (pack.weight != null) pack.weight = String(pack.weight).replace(/[a-zA-Z]/g, '')
		const weight = Number(pack.weight)
	...
	const calcPriceWithVolume = (pack: QuickQuotePackage): void => {
		if (pack.first_size) pack.first_size = String(pack.first_size).replace(/[^0-9]/g, '')
		if (pack.second_size) pack.second_size = String(pack.second_size).replace(/[^0-9]/g, '')
		if (pack.third_size) pack.third_size = String(pack.third_size).replace(/[^0-9]/g, '')
	...
	const selectPackageType = (packageType: unknown): void => {
	...
	const addPackageInline = (packageType?: unknown): void => {
	...
	const updatePackageType = (pack: QuickQuotePackage, packageType: unknown): void => {
		ensurePackageDraftIdentity(pack, packageType)
	}

	const deletePack = async (targetPackId: string | number): Promise<void> => {
