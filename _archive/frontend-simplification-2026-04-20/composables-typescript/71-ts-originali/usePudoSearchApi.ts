...
interface PudoSearchProps {
	initialCity?: string
	initialZip?: string
	[key: string]: unknown
}

type PudoSearchEmit = (event: string, ...args: unknown[]) => void
...
interface GeocodeFromFieldsResult {
	latitude: number
	longitude: number
	label: string
}

interface MapReferenceClickPayload {
...
export function usePudoSearchApi(props: PudoSearchProps, emit: PudoSearchEmit) {
	...
	const getErrorStatus = (error: unknown): number => {
		const err = error as { status?: number; response?: { status?: number }; data?: { statusCode?: number } } | undefined
		return Number(err?.status ?? err?.response?.status ?? err?.data?.statusCode ?? 0)
	}
	const getErrorMessage = (error: unknown): string => {
		...
		return err?.data?.error || err?.data?.message || err?.response?._data?.message || err?.message || ''
	}

	// ── Search state ──
	const searchAddress = ref<string>('')
	const searchCity = ref<string>(props.initialCity || '')
	const searchZip = ref<string>(props.initialZip || '')
	const loading = ref<boolean>(false)
	const geolocating = ref<boolean>(false)
	const searched = ref<boolean>(false)
	const searchError = ref<string | null>(null)
	const searchMeta = ref<BrtPudoMeta | null>(null)
	const referenceUpdateMessage = ref<string>('')

	// ── Results state ──
	const pudoResults = ref<BrtPudoNormalized[]>([])
	...
	const mapClickLoading = ref<boolean>(false)
	...
	const extractLatitude = (point: Record<string, unknown> = {}): number | null => {
		const p = point as Record<string, unknown>
		const nested = (key: string) => (p[key] as Record<string, unknown> | undefined)
		return parseCoordinate(
			p.latitude ?? p.lat ?? nested('coordinates')?.latitude ?? nested('coordinates')?.lat
	...
	const extractLongitude = (point: Record<string, unknown> = {}): number | null => {
		const p = point as Record<string, unknown>
		const nested = (key: string) => (p[key] as Record<string, unknown> | undefined)
		return parseCoordinate(
			p.longitude ?? p.lng ?? p.lon ?? nested('coordinates')?.longitude ?? nested('coordinates')?.lng
	...
	const parseDistanceMeters = (value: unknown): number | null => {
		if (value === null || value === undefined || value === '') return null
	...
	const isFiniteCoordinate = (value: unknown): boolean => Number.isFinite(parseCoordinate(value))
	const normalizeTextKey = (value: unknown): string => String(value || '').trim().toLowerCase()

	// ── Computed ──
	const hasSearchInput = computed<boolean>(() => Boolean(searchCity.value?.trim() || searchZip.value?.trim()))
	const mapPoints = computed<BrtPudoNormalized[]>(() =>
		pudoResults.value.filter((p) => isFiniteCoordinate(p.latitude) && isFiniteCoordinate(p.longitude)),
	)
	const mapReferencePoint = computed<MapReferencePoint | null>(() => {
	...
	const strategyListLabel = computed<string>(() => {
	...
	const getPudoUiKey = (point: Record<string, unknown> | null | undefined): string => {
		const p = point || {}
		const primary = String((p as Record<string, unknown>).pudo_id || (p as Record<string, unknown>).carrier_pudo_id || (p as Record<string, unknown>).id || '').trim()
		if (primary) return primary
		const lat = extractLatitude(p as Record<string, unknown>)
		const lng = extractLongitude(p as Record<string, unknown>)
		return [
			normalizeTextKey((p as Record<string, unknown>).name),
			normalizeTextKey((p as Record<string, unknown>).address),
			normalizeTextKey((p as Record<string, unknown>).zip_code),
			normalizeTextKey((p as Record<string, unknown>).city),
			Number.isFinite(lat as number) ? (lat as number).toFixed(6) : 'na',
			Number.isFinite(lng as number) ? (lng as number).toFixed(6) : 'na',
		].join('|')
	}

	const normalizePudoPoint = (rawPoint: unknown): BrtPudoNormalized => {
		...
		return {
			...
			name: (point.name as string) || 'Punto di ritiro BRT',
			address: (point.address as string) || '',
			city: (point.city as string) || '',
			zip_code: (point.zip_code as string) || '',
			province: (point.province as string) || '',
			country: (point.country as string) || 'ITA',
			latitude,
			longitude,
			distance_meters: distance,
			enabled: typeof point.enabled === 'boolean' ? point.enabled : true,
			opening_hours: (point.opening_hours as string | null) ?? null,
			localization_hint: (point.localization_hint as string) || '',
		}
	}

	const dedupePudoPoints = (points: BrtPudoNormalized[]): BrtPudoNormalized[] => {
		const byKey = new Map<string, BrtPudoNormalized>()
		points.forEach((point) => {
			const key = getPudoUiKey(point as unknown as Record<string, unknown>)
	...
	const sortByDistance = (points: BrtPudoNormalized[]): BrtPudoNormalized[] =>
		[...points].sort((a, b) => {
	...
	const inferReferenceFromResults = (points: BrtPudoNormalized[] = []): ReferencePoint | null => {
		const coords = points.map((p) => ({
			latitude: parseCoordinate(p?.latitude ?? (p as unknown as { lat?: number }).lat),
			longitude: parseCoordinate(p?.longitude ?? (p as unknown as { lng?: number }).lng),
		}))
			.filter((c): c is { latitude: number; longitude: number } => Number.isFinite(c.latitude) && Number.isFinite(c.longitude))
	...
	const applyResults = (rawPoints: unknown[] | null | undefined): void => {
		...
		const withDist = normalized.map((point): BrtPudoNormalized => {
			...
			if (distRef && Number.isFinite(point.latitude) && Number.isFinite(point.longitude)) {
				const computed = distanceInMeters(distRef, { latitude: point.latitude as number, longitude: point.longitude as number })
				const shouldReplace = !hasApi || apiD <= 0 || allZero
				if (shouldReplace && Number.isFinite(computed as number)) return { ...point, distance_meters: computed }
				if (hasApi && apiD > 0 && Number.isFinite(computed as number) && Math.abs(apiD - (computed as number)) > 200000) return { ...point, distance_meters: computed }
	...
	// ── Geocoding ──
	const fetchWithTimeout = async (url: string, options: RequestInit = {}, timeoutMs: number = 4500): Promise<Response> => {
	...
	const geocodeFromSearchFields = async (): Promise<GeocodeFromFieldsResult | null> => {
		...
		const payload = (await response.json()) as NominatimSearchResult[] | null
	...
	const reverseGeocodeFromCoordinates = async (
		...
		try {
			...
			if (!response.ok) return null
			const payload = (await response.json()) as NominatimReverseResult | null
	...
	// ── PUDO API calls ──
	const fetchNearbyPudo = async (latitude: number, longitude: number, maxResults: number = 50): Promise<unknown[]> => {
		const params = new URLSearchParams()
		params.set('latitude', String(latitude))
		params.set('longitude', String(longitude))
		params.set('max_results', String(maxResults))
		const result = await publicApiFetch<BrtPudoResponse>(`/api/brt/pudo/nearby?${params.toString()}`)
		return result?.pudo || result?.data?.pudo || []
	}

	const searchPudo = async (): Promise<void> => {
		...
		try {
			...
			const result = await publicApiFetch<BrtPudoResponse>(`/api/brt/pudo/search?${params.toString()}`)
			if (result?.success === false) { searchError.value = result?.error || 'Errore durante la ricerca dei punti di ritiro.'; return }

			let points: unknown[] = result?.pudo || result?.data?.pudo || []
	...
	const useCurrentLocation = async (): Promise<void> => {
		...
		try {
			...
			const status = getErrorStatus(error)
			const geoCode = Number((error as { code?: number } | undefined)?.code || 0)
	...
	// ── Map click handler ──
	const onMapReferenceClick = async (payload: MapReferenceClickPayload): Promise<void> => {
		...
		try {
			...
			loading.value = true
			searched.value = true
			const nearbyPoints = await fetchNearbyPudo(lat as number, lng as number, 50)
			applyResults(nearbyPoints)
			searchMeta.value = { strategy_used: ['nearby_geo'], returned_count: pudoResults.value.length, requested_count: 50, provider: 'BRT', fallback: false }
		} catch {
	...
	// ── PUDO detail fetch ──
	const fetchPudoDetails = async (pudo: BrtPudoNormalized, detailKey: string): Promise<void> => {
		...
		try {
			const result = (await publicApiFetch(`/api/brt/pudo/${pudo.pudo_id}`)) as Record<string, unknown> | null
			const pudoField = result?.pudo as unknown
			const dataField = result?.data as Record<string, unknown> | undefined
			const dataPudo = dataField?.pudo as unknown
			const p: Record<string, unknown> =
				((pudoField && typeof pudoField === 'object' ? (pudoField as Record<string, unknown>) : undefined)
				|| (dataPudo && typeof dataPudo === 'object' ? (dataPudo as Record<string, unknown>) : undefined)
				|| (dataField as Record<string, unknown> | undefined)
				|| (result as Record<string, unknown> | null)
				|| {})
			pudoDetails.value[detailKey] = {
				opening_hours: ((p.opening_hours as string | null | undefined) ?? (p.hours as string | null | undefined) ?? pudo.opening_hours ?? '') || null,
				localization_hint: String((p.localization_hint ?? p.localizationHint ?? pudo.localization_hint) ?? ''),
				enabled: typeof p.enabled === 'boolean' ? (p.enabled as boolean) : pudo.enabled,
			}
		} catch (error) {
			const status = getErrorStatus(error)
