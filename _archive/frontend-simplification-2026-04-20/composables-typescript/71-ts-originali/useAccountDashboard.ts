...
import { createAccountSections } from '~/utils/accountNavigation';
...
export function useAccountDashboard() {
	...
	const sanctum: any = useSanctumClient();
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const { user, logout } = useSanctumAuth() as any;
	...
	const resolveRoleKey = (...candidates: Array<string | null | undefined>) => {
	...
	const roleKey = computed(() => resolveRoleKey(uiSnapshot.value.role, user.value?.role));
	...
	const visibleSections = computed(() =>
		sections.value
			...
			.map((section: any) => ({
				...section,
				// eslint-disable-next-line @typescript-eslint/no-explicit-any
				pages: section.pages.filter((page: any) => page.visible),
			}))
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			.filter((section: any) => section.pages.length > 0),
	);

	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const allVisiblePages = computed(() => visibleSections.value.flatMap((section: any) => section.pages));
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const customerOrders = computed(() => (((orders.value || {}) as any).data || []));
	...
	const customerCompletedCount = computed(() =>
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		(Array.isArray(customerOrders.value) ? customerOrders.value : []).filter((order: any) =>
	...
	const highlightedCustomerOrders = computed(() => {
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		const openOrders = list.filter((order: any) => customerActiveStatusIds.includes(statusRaw(order?.status)));
		...
		return source.map((order: any) => {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const statusLabel = (orderStatusConfig as any)[rawStatus]?.label || order?.status || 'Aggiornato';
	...
	const recentCompletedCustomerOrders = computed(() => {
		...
		return list
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			.filter((order: any) => customerCompletedStatusIds.includes(statusRaw(order?.status)))
			.slice(0, 3)
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			.map((order: any) => ({
				id: order?.id,
	...
	/* ---------------------- ADMIN DASHBOARD ---------------------- */
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const adminDashboardData = ref<any>(null);
	...
	const adminKpis = computed(() => {
		...
		return [
			...
			},
			{
				...
				url: '/account/amministrazione/portafogli',
				iconKey: 'wallet',
				iconBg: '#FFF4EE',
				iconColor: '#E44203',
			},
	...
	const adminRecentItems = computed(() => {
		...
		return recentOrders.slice(0, 6).map((order: any) => {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			const statusLabel = (orderStatusConfig as any)[order.status]?.label || order.status || 'Aggiornato';
