...
interface UseAdminPrezziFormOptions {
...
type BandField = 'base_price' | 'discount_price'
...
export const useAdminPrezziForm = ({
	...
	supplementRules,
	showError,
}: UseAdminPrezziFormOptions) => {
	// ── Editing state ────────────────────────────────────
	const editingCell = ref<string | null>(null);
	const editValue = ref<string>('');

	// ── Utility ──────────────────────────────────────────
	const centsToEuro = (cents: number | string | null | undefined): string => {
		if (cents == null || cents === '') return '-';
		return (Number(cents) / 100).toFixed(2).replace('.', ',') + '\u20AC';
	};

	const euroToCents = (euro: number | string | null | undefined): number | null => {
	...
	const incrementCentsToEuro = (value: number | string | null | undefined): string => (Number(value || 0) / 100).toFixed(2).replace('.', ',');

	const updateLadderIncrementFromEuro = (row: LadderRow, rawValue: string | number): void => {
		const cents = euroToCents(rawValue);
		row.increment_cents = Math.max(0, cents ?? 0);
	};

	// ── Ladder helpers ───────────────────────────────────
	const normalizeLadderForPayload = (rows: Partial<LadderRow>[] | null | undefined, fallbackIncrement: number | string | null | undefined): LadderRow[] => {
		...
		const normalized = source
			.map((row, idx) => {
				...
				const toStep = toRaw === null || toRaw === undefined || (toRaw as unknown) === ''
					? null
	...
	const removeLadderRow = (kind: BandKind, idx: number): void => {
		const rows = ladderRowsFor(kind);
		if (rows.length <= 1) {
	...
	const ensureLadderContinuity = (kind: BandKind): void => {
		...
		const rebuilt: LadderRow[] = normalized.map((row, idx) => {
			const prev = normalized[idx - 1];
			return {
				from_step: idx === 0 ? 1 : ((prev?.to_step as number | null) ?? 0) + 1,
	...
	const effectivePriceCentsLocal = (band: PriceBand | null | undefined): number => {
		if (!band) return 0;
	...
	const ceilByResolutionLocal = (value: number | string, resolution: number | string): number => {
	...
	const findBandLocal = (bands: PriceBand[] | null | undefined, rawValue: number | string): PriceBand | null => {
	...
	const calculateExtraPriceCentsLocal = (type: BandKind, rawValue: number | string): number | null => {
		...
		if (extraRules.value.base_price_cents_mode === 'manual') {
			...
			const lastBand = sourceBands[sourceBands.length - 1] ?? null;
	...
	const calculateBandPriceCentsLocal = (type: BandKind, rawValue: number | string): number => {
		...
		const lastBand = bands[bands.length - 1] ?? null;
		return effectivePriceCentsLocal(lastBand);
	};

	// ── Band edit actions ────────────────────────────────
	const startEdit = (type: BandKind, idx: number, field: BandField): void => {
		const key = `${type}-${idx}-${field}`;
		editingCell.value = key;
		const bands = type === 'weight' ? weightBands.value : volumeBands.value;
		const currentBand = bands[idx];
		if (!currentBand) return;
		const cents = (currentBand as unknown as Record<string, number | null>)[field];
	...
	const confirmEdit = (type: BandKind, idx: number, field: BandField): void => {
		const key = `${type}-${idx}-${field}`;
		...
		(currentBand as unknown as Record<string, number | null>)[field] = newCents;
		editingCell.value = null;
		editValue.value = '';
	};
	...
	const removeBand = (type: BandKind, idx: number): void => {
		const bands = type === 'weight' ? weightBands.value : volumeBands.value;
		if (bands.length <= 1) {
	...
	const moveBand = (type: BandKind, idx: number, direction: number): void => {
		const bands = type === 'weight' ? weightBands.value : volumeBands.value;
		const target = idx + direction;
		if (target < 0 || target >= bands.length) return;
		const source = bands[idx];
		const destination = bands[target];
		if (!source || !destination) return;
		[bands[idx], bands[target]] = [destination, source];
	};
	...
	const supplementAmountToEuro = (rule: SupplementRule | null | undefined): string => {
		const cents = Number(rule?.amount_cents || 0);
		return (cents / 100).toFixed(2).replace('.', ',');
	};

	const updateSupplementAmountFromEuro = (rule: SupplementRule, rawValue: string | number): void => {
	...
	const keyedRuleAmountToEuro = (rule: PricingRule | null | undefined): string => (Number(rule?.price_cents || 0) / 100).toFixed(2).replace('.', ',');

	const updateKeyedRuleAmountFromEuro = (rule: PricingRule, rawValue: string | number): void => {
		const cents = euroToCents(rawValue);
		rule.price_cents = Math.max(0, cents ?? 0);
	};

	const keyedRuleMinFeeToEuro = (rule: PricingRule | null | undefined): string => (Number(rule?.min_fee_cents || 0) / 100).toFixed(2).replace('.', ',');

	const updateKeyedRuleMinFeeFromEuro = (rule: PricingRule, rawValue: string | number): void => {
		const cents = euroToCents(rawValue);
		rule.min_fee_cents = Math.max(0, cents ?? 0);
	};

	const normalizeArrayFieldInput = (rawValue: string | null | undefined, { uppercase = false }: { uppercase?: boolean } = {}): string[] =>
		String(rawValue || '')
			.split(',')
	...
	const updateArrayField = (rule: PricingRule, field: string, rawValue: string, { uppercase = false }: { uppercase?: boolean } = {}): void => {
		(rule as Record<string, unknown>)[field] = normalizeArrayFieldInput(rawValue, { uppercase });
	};

	const addTierRow = (rule: PricingRule): void => {
	...
	// ── Europe rate helpers ──────────────────────────────
	const updateEuropeRateAmountFromEuro = (rate: EuropeRate, rawValue: string | number): void => {
		const cents = euroToCents(rawValue);
		rate.price_cents = cents == null ? null : Math.max(0, cents);
	};

	const toggleEuropeRateQuote = (rate: EuropeRate): void => {
