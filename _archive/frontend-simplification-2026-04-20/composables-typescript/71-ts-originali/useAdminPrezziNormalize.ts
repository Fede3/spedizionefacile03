// useAdminPrezziNormalize — normalizzatori e payload builders per Admin Prezzi.
// Include normalizzazione stringhe/tier/gruppi pricing/Europa + serializzazione rules.
import type {
	EuropePricing,
	PricingRule,
	PricingRuleGroup,
} from '~/types';
import { DEFAULT_EUROPE_PRICING } from './usePriceBandsDefaults';

interface PartialTier {
	up_to_kg?: number | string | null;
	price_cents?: number | string;
}

export const normalizeStringListForAdmin = (
	values: unknown[] = [],
...
export const normalizeTiersForAdmin = (tiers: PartialTier[] = []): { up_to_kg: number | null; price_cents: number }[] => {
...
export const normalizePricingGroupForAdmin = (
	config: PricingRuleGroup = {},
	defaults: PricingRuleGroup = {},
): PricingRuleGroup => Object.fromEntries(
	Object.entries(defaults).map(([key, fallback]) => {
		const source = config?.[key] && typeof config[key] === 'object' ? config[key] as PricingRule : {} as PricingRule;
		return [key, {
			...fallback,
			...source,
...
export const normalizeEuropePricingForAdmin = (config: Partial<EuropePricing> = {}): EuropePricing => {
		...
		? config.bands.map((band, bandIndex) => ({
			...
			rates: Array.isArray(band?.rates)
				? band.rates.map((rate) => ({
					...
					price_cents: rate?.price_cents === null || (rate?.price_cents as unknown) === '' || rate?.price_cents === undefined
						? null
...
export const buildPricingRulesPayload = (group: PricingRuleGroup = {}): PricingRuleGroup =>
