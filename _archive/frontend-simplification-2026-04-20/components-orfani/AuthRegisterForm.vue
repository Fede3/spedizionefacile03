...
watchEffect(() => {
	if (props.registerForm?.role !== 'Cliente') {
		props.registerForm.role = 'Cliente'
	}
})
</script>
...
<template>
	<form method="post" @submit.prevent="submitRegister" class="auth-overlay-form auth-page-form">
		<div class="auth-choice-group auth-choice-group--secondary auth-choice-group--register-profile">
			<div class="auth-choice-radio-row" role="radiogroup" aria-label="Tipo account">
				<label
					:class="[
						'auth-choice-radio',
						registerForm.user_type === 'privato'
							? 'auth-choice-radio--active'
							: '',
					]">
					<input type="radio" value="privato" v-model="registerForm.user_type" class="sr-only" />
					<span class="auth-choice-radio__dot" aria-hidden="true"></span>
					<span>Privato</span>
				</label>
				<label
					:class="[
		...
		<div v-if="registerForm.referred_by" class="auth-feedback auth-feedback--success">
			<p class="font-semibold">Codice invito applicato: <strong>{{ registerForm.referred_by }}</strong></p>
		...
		<label class="auth-privacy-consent">
			...
			<span class="auth-privacy-consent__text">
				...
				e i
				<NuxtLink to="/termini-e-condizioni" target="_blank" class="auth-privacy-consent__link">Termini e Condizioni</NuxtLink> *
			</span>
		</label>
