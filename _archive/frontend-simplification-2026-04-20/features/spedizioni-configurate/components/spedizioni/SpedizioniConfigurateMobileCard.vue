...
<template>
	<div :class="['desktop:hidden p-[14px] tablet:p-[16px] border-b border-[var(--color-brand-border)] bg-white', isDuplicate ? 'bg-amber-50 border-l-[3px] border-l-amber-400' : '']">
		<div class="flex items-start justify-between gap-[10px]">
			<div class="flex items-start gap-[8px] min-w-0">
				<input type="checkbox" :checked="isSelected" @change="emit('toggle', item.id)" class="w-[16px] h-[16px] accent-[var(--color-brand-primary)] cursor-pointer" />
				<div class="min-w-0">
					<p class="text-[0.875rem] font-semibold text-[var(--color-brand-text)] leading-[1.25]">{{ item.origin_address?.city || 'Partenza' }} &rarr; {{ item.destination_address?.city || 'Dest.' }}</p>
					<p class="mt-[3px] text-[0.75rem] text-[var(--color-brand-text-secondary)] leading-[1.35]">{{ item.quantity }}x - {{ item.weight }} kg - {{ formatCreatedDate(item) }}</p>
			...
			<span class="shrink-0 text-[0.9375rem] font-bold text-[var(--color-brand-text)]">{{ formatPrice(item.single_price) }}</span>
		</div>
		<div class="mt-[10px] flex flex-wrap gap-[8px]">
			<span class="inline-flex items-center gap-[6px] rounded-full bg-[#F8F9FB] px-[10px] py-[5px] text-[0.75rem] font-medium text-[var(--color-brand-text)]">
				<NuxtImg :src="getPackageIcon(item)" :alt="item.package_type || 'Tipo collo'" width="16" height="18" loading="lazy" decoding="async" class="shrink-0" />
			...
			<span class="inline-flex items-center gap-[6px] rounded-full bg-[#EAF4F6] px-[10px] py-[5px] text-[0.75rem] font-medium text-[var(--color-brand-primary)]">
		...
		<div class="mt-[12px] grid grid-cols-1 sm:grid-cols-2 gap-[8px]">
			<div class="rounded-[16px] bg-[#F8F9FB] px-[12px] py-[10px]">
		...
		<div class="mt-[12px] flex flex-wrap gap-[10px] justify-end">
			<button type="button" @click="emit('edit', item)" class="inline-flex items-center gap-[4px] rounded-full border border-[var(--color-brand-border)] bg-white px-[12px] py-[7px] text-[0.75rem] text-[var(--color-brand-primary)] font-semibold hover:border-[var(--color-brand-primary)] cursor-pointer">
			...
			<button type="button" @click="emit('delete', item.id)" class="inline-flex items-center gap-[4px] rounded-full border border-red-200 bg-white px-[12px] py-[7px] text-[0.75rem] text-red-600 font-semibold hover:border-red-300 cursor-pointer" aria-label="Elimina spedizione" title="Elimina">
