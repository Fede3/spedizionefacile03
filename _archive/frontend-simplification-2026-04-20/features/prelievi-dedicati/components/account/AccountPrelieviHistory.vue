...
<template>
	<div class="rounded-[16px] bg-white p-[16px] desktop:p-[20px]" style="box-shadow: 0 2px 8px rgba(9,88,102,0.06), 0 0 0 1px rgba(9,88,102,0.04);">
		<div class="flex items-center gap-[12px] mb-[18px] desktop:mb-[18px]">
			<div class="w-[36px] h-[36px] rounded-[50px] bg-[#edf7f8] flex items-center justify-center">
				<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="var(--color-brand-primary)">
					<path
			...
			<h2 class="font-montserrat text-[1rem] font-[800] text-[var(--color-brand-text)]">Storico prelievi</h2>
		</div>

		<!-- Loading -->
		<div v-if="isLoadingData" class="py-[24px] flex justify-center">
			<div class="w-[30px] h-[30px] border-3 border-[var(--color-brand-border)] border-t-[var(--color-brand-primary)] rounded-full animate-spin"></div>
		</div>
		...
		<div v-else-if="!withdrawals?.length" class="text-center py-[32px]">
			<div class="w-[56px] h-[56px] mx-auto mb-[16px] rounded-full bg-[#F5F6F9] flex items-center justify-center">
				<svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="#C8CCD0">
					<path
			...
			<p class="text-[0.9375rem] font-medium text-[var(--color-brand-text)]">Ancora nessuna richiesta</p>
			<p class="text-[0.75rem] text-[var(--color-brand-text-secondary)] mt-[6px]">Qui appariranno le richieste.</p>
		</div>
		...
		<div v-else class="space-y-[14px]">
			<div
				...
				:key="withdrawal.id"
				:class="[
					'p-[16px] rounded-[16px] border transition-colors',
					statusConfig[withdrawal.status]?.border || 'border-[var(--color-brand-border)]',
				...
				<div class="flex flex-col gap-[10px] tablet:flex-row tablet:items-start tablet:justify-between">
					<div class="min-w-0">
						<div class="flex items-center gap-[8px] flex-wrap">
							<span class="text-[1rem] font-bold text-[var(--color-brand-text)]">&euro;{{ formatEuro(withdrawal.amount) }}</span>
							<span
								:class="[
									'inline-flex items-center gap-[4px] px-[9px] py-[3px] rounded-full text-[0.6875rem] font-medium',
									statusConfig[withdrawal.status]?.text || 'text-[var(--color-brand-text-secondary)]',
								]">
						...
						<p class="mt-[6px] text-[0.75rem] font-medium text-[var(--color-brand-text-secondary)]">Richiesta #{{ withdrawal.id }}</p>
					</div>
					<div class="text-[0.6875rem] text-[var(--color-brand-text-secondary)] tablet:text-right">
						<p class="font-medium">Inviata il</p>
						<p class="mt-[2px] whitespace-nowrap">{{ formatDate(withdrawal.created_at) }}</p>
					</div>
				</div>
				<p v-if="withdrawal.admin_notes" class="text-[0.75rem] text-[var(--color-brand-text-secondary)] italic mt-[4px]">Note: {{ withdrawal.admin_notes }}</p>
				<p v-if="withdrawal.reviewed_at" class="text-[0.6875rem] text-[var(--color-brand-text-secondary)] mt-[4px]">
