/**
 * COMPOSABLE: useAddressFormState (useAddressFormState.js)
 * SCOPO: Gestisce lo stato reattivo del form indirizzi (origine/destinazione),
 *        inclusi pre-fill da session/userStore e watchers per sincronizzazione.
 *
 * DOVE SI USA: importato da useAddressForm.js (orchestratore)
 */
import { ref, computed, watch } from "vue";

export function useAddressFormState({ session, isAuthenticated, userStore, route, deliveryMode }) {
  ...
  // --- PUDO (Punto di ritiro BRT) delivery mode ---
  const deliveryModeComputed = computed({
  ...
  return {
    ...
    canSaveDestAddress,
    deliveryMode: deliveryModeComputed,
    isAuthenticated,
  };
}
