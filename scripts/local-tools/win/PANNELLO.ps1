...
# Repository root (3 livelli sopra rispetto a scripts/local-tools/win/)
$repoRoot = (Resolve-Path (Join-Path $root "..\..\..")).Path
$runtimeStateDir = Join-Path $repoRoot "output\runtime-state"
$state   = Join-Path $runtimeStateDir "_STATE.json"
$logDir  = Join-Path $root "_LOG"
$urlFile = Join-Path $runtimeStateDir "URL_ONLINE.txt"

if(-not (Test-Path $logDir)){ New-Item -ItemType Directory -Path $logDir | Out-Null }
if(-not (Test-Path $runtimeStateDir)){ New-Item -ItemType Directory -Path $runtimeStateDir -Force | Out-Null }
...
function Resolve-ProjectDir([string]$preferredName, [string[]]$markerFiles, [string]$projectLabel){
  $preferred = Join-Path $repoRoot $preferredName
  foreach($marker in $markerFiles){
    if(Test-Path (Join-Path $preferred $marker)){ return $preferred }
  }

  $dirs = Get-ChildItem -Path $repoRoot -Directory -ErrorAction SilentlyContinue
  $matches = @()
  foreach($d in $dirs){
    foreach($marker in $markerFiles){
...
function Has-Caddyfile(){ return (Test-Path (Join-Path $repoRoot "Caddyfile")) }
...
function Start-Local([switch]$NonAprireBrowser){
  ...
  $pFront = Start-Process -FilePath "cmd.exe" -WorkingDirectory $frontDir -PassThru -WindowStyle Hidden `
    -ArgumentList "/c","set NODE_OPTIONS=--max-old-space-size=6144 && npx nuxi dev --host 127.0.0.1 --port $frontPort" `
  ...
  if(Has-Caddyfile){
    if(Has-Caddy){
      ...
      $p = Start-Process -FilePath "cmd.exe" -WorkingDirectory $repoRoot -PassThru -WindowStyle Hidden `
