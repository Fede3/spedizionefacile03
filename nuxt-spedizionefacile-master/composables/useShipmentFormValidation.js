 ...
 *
 * Gestisce la validazione a livello di form per lo step indirizzi della spedizione:
 * - validateForm: validazione completa del form (tutti i campi + coerenza location)
 * - Error summary, grouping, section hints
 * - Focus helpers (focusFormError, focusFirstFormError, focusContentDescriptionField)
 * - Field error display helpers (getFieldError, fieldClass, fieldErrorText)
 * - softenErrorMessage: rende i messaggi di errore piu' user-friendly
 * - Field assist (suggerimenti di correzione automatica per ogni campo)
 * - Utility: normalizeSimpleText, buildEmailSuggestion, extractAddressAndNumber
 *
...
export const useShipmentFormValidation = ({
	...
	// dedupeLocations and getProvinceLabel are auto-imported from ~/utils/location.js
