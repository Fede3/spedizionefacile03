...
export function useWalletTopUp(props, emit) {
  ...
  const stripeErrorMsg = (err) =>
    STRIPE_ERRORS[err?.code] || err?.message || "Errore durante il salvataggio della carta. Riprova.";
  ...
  const mountNewCardFields = async () => {
    ...
    const style = {
      base: { color: "#252B42", fontFamily: '"Inter", sans-serif', fontSize: "15px", fontWeight: "400", "::placeholder": { color: "#a0a0a0" } },
  ...
  const openNewCardForm = async () => {
    ...
    if (!(await ensureStripeLoaded())) { isPreparingNewCardForm.value = false; return; }

    try {
      const res = await sanctum("/api/stripe/create-setup-intent", { method: "POST" });
      if (!res?.client_secret) { cardError.value = res?.error || "Impossibile inizializzare il modulo carta. Riprova."; return; }
  ...
  const closeNewCardForm = () => { showNewCardForm.value = false; clearNewCardForm(); };

  /* ── Save new card ── */

  const saveNewCardAndGetPaymentMethodId = async () => {
    if (!setupClientSecret.value) { cardError.value = "Impossibile inizializzare il modulo carta. Riprova."; return null; }
    if (!cardHolderName.value.trim()) { cardError.value = "Inserisci il nome del titolare della carta."; return null; }
    if (!(await ensureStripeLoaded()) || !stripe) { cardError.value = "Stripe non disponibile. Ricarica la pagina e riprova."; return null; }
    ...
    if (error) { cardError.value = stripeErrorMsg(error); return null; }
    if (!setupIntent?.payment_method) { cardError.value = "Metodo di pagamento non trovato. Riprova."; return null; }

    const srv = await sanctum("/api/stripe/set-default-payment-method", {
      method: "POST", body: { payment_method: setupIntent.payment_method },
    });
    if (srv?.error) { cardError.value = srv.error || "Errore durante il salvataggio della carta."; return null; }
  ...
  const handleTopUp = async () => {
    if (!topUpAmount.value || topUpAmount.value < 1) { setFeedback("Inserisci un importo minimo di 1,00 EUR"); return; }
    if (!props.stripeConfigured) { setFeedback("Le ricariche con carta non sono ancora attive su questo sito."); return; }
    ...
    try {
      ...
      if (!pmId) { setFeedback("Aggiungi e salva una carta valida per completare la ricarica."); return; }

      const result = await sanctum("/api/wallet/top-up", {
        method: "POST", body: { amount: Number(topUpAmount.value), payment_method_id: pmId },
      });
