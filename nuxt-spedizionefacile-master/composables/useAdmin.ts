...
interface MyMoneyObject {
	amount: number | string
	formatted?: string
}

type CurrencyValue = MyMoneyObject | string | number | null | undefined

interface OrderLike {
	id: number | string
	brt_parcel_id?: string | null
...
export const useAdmin = () => {
	...
	const showError = (e: unknown, fallback: string): void => {
		const errObj = e as { response?: { _data?: { message?: string } }, data?: { message?: string } }
		actionMessage.value = {
			type: 'error',
	...
	const formatCurrency = (val: CurrencyValue): string => {
	...
	const formatCents = (val: number | string | null | undefined): string =>
		formatEuro(Number(val || 0) / 100)

	/* Formatta una data nel formato italiano con ora */
	const formatDate = (dateStr: string | Date | null | undefined): string =>
		formatDateTimeIt(dateStr, '\u2014')
	...
	const downloadLabel = async (order: OrderLike): Promise<void> => {
