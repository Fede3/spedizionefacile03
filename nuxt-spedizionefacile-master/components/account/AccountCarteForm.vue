...
<template>
  <div class="bg-white rounded-[20px] p-[18px] tablet:p-[20px] desktop:p-[28px] shadow-sm border border-[var(--color-brand-border)] max-w-[760px] mx-auto">
    <div class="mb-[16px]">
    ...
    <div class="mb-[16px]">
      ...
      <input
        ...
        class="w-full px-[14px] py-[11px] bg-[#F5F6F9] border border-[var(--color-brand-border)] rounded-[14px] text-[0.875rem] text-[var(--color-brand-text)] placeholder:text-[var(--color-brand-text-muted)] focus:border-[var(--color-brand-primary)] focus:outline-none transition-colors"
    ...
    <p v-if="errorMessage" class="text-red-500 text-[0.75rem] mb-[16px] p-[10px] bg-red-50 rounded-[14px] border border-red-200">
      {{ errorMessage }}
    </p>
    ...
    <div class="flex flex-col sm:flex-row gap-[10px]">
      ...
      <button @click="emit('save')"
        class="btn-cta btn-compact flex-1 inline-flex items-center justify-center gap-[6px] py-[12px] font-semibold text-[0.875rem] cursor-pointer">
