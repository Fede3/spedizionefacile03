	...
	<div class="grid grid-cols-1 desktop:grid-cols-[minmax(0,1fr)_420px] gap-[14px] desktop:gap-[18px]">
		...
		<div class="bg-white rounded-[24px] p-[18px] tablet:p-[22px] desktop:p-[28px] shadow-sm border border-[#E9EBEC] h-full">
			<div class="flex flex-col tablet:flex-row tablet:items-start gap-[16px]">
				<div class="w-[64px] h-[64px] shrink-0 rounded-[12px] border border-[#C7D8DE] bg-[#F4FAF9] flex items-center justify-center">
			...
			<div class="mt-[18px] grid grid-cols-1 sm:grid-cols-3 gap-[10px]">
				<div class="rounded-[12px] border border-[#E9EBEC] bg-[#FAFCFD] p-[14px]">
			...
			<!-- Pending -->
			<div v-if="hasRequest && statusType === 'pending'" class="mt-[18px] bg-amber-50 border border-amber-200 rounded-[12px] p-[16px] tablet:p-[18px]">
			...
			<!-- Approved -->
			<div v-else-if="hasRequest && statusType === 'approved'" class="mt-[18px] bg-emerald-50 border border-emerald-200 rounded-[12px] p-[16px] tablet:p-[18px]">
				<div class="flex items-start gap-[10px]">
			...
			<!-- Rejected -->
			<div v-else-if="hasRequest && statusType === 'rejected'" class="mt-[18px] bg-red-50 border border-red-200 rounded-[12px] p-[16px] tablet:p-[18px]">
				<div class="flex items-start gap-[10px]">
			...
			<div v-else class="space-y-[14px]">
				...
				<div class="space-y-[10px]">
					<div>
						...
						<input id="pro_company_name" :value="proRequestForm.company_name" @input="updateField('company_name', $event.target.value)" type="text" placeholder="Nome azienda" class="w-full min-h-[48px] px-[14px] rounded-[12px] border border-[#D7E1E4] bg-white text-[#252B42] text-[0.9375rem] outline-none focus:border-[#095866] focus:ring-2 focus:ring-[#095866]/10" />
					</div>
					<div>
						<label class="block text-[0.8125rem] font-semibold text-[#252B42] mb-[6px]" for="pro_vat_number">Partita IVA</label>
						<input id="pro_vat_number" :value="proRequestForm.vat_number" @input="updateField('vat_number', $event.target.value)" type="text" placeholder="Partita IVA" class="w-full min-h-[48px] px-[14px] rounded-[12px] border border-[#D7E1E4] bg-white text-[#252B42] text-[0.9375rem] outline-none focus:border-[#095866] focus:ring-2 focus:ring-[#095866]/10" />
					</div>
					<div>
						<label class="block text-[0.8125rem] font-semibold text-[#252B42] mb-[6px]" for="pro_message">Messaggio</label>
						<textarea id="pro_message" :value="proRequestForm.message" @input="updateField('message', $event.target.value)" rows="4" placeholder="Breve nota opzionale" class="w-full px-[14px] py-[12px] rounded-[12px] border border-[#D7E1E4] bg-white text-[#252B42] text-[0.9375rem] outline-none focus:border-[#095866] focus:ring-2 focus:ring-[#095866]/10 resize-none"></textarea>
					</div>
				</div>
