...
<template>
	<!-- Summary cards -->
	<div class="mb-[16px] grid gap-[12px] desktop:grid-cols-[minmax(0,1.35fr)_minmax(280px,0.65fr)]">
		<div class="rounded-[20px] border border-[var(--color-brand-border)] bg-white px-[16px] py-[14px] shadow-sm">
			<p class="text-[0.75rem] font-semibold uppercase tracking-[1px] text-[var(--color-brand-primary)]">Vista rubrica</p>
			<h2 class="mt-[6px] font-montserrat text-[1.125rem] font-[800] text-[var(--color-brand-text)]">Indirizzi ordinati e pronti all&apos;uso</h2>
		...
		<div class="rounded-[20px] border border-[#DDECEE] bg-[#F8FCFD] px-[16px] py-[14px] shadow-sm">
	...
	<div v-if="!addresses" class="space-y-[12px]">
		<div v-for="n in 2" :key="n" class="bg-white rounded-[20px] p-[24px] border border-[var(--color-brand-border)] animate-pulse">
	...
	<!-- Empty state -->
	<div v-else-if="addresses?.data?.length === 0" class="bg-white rounded-[20px] p-[48px] border border-[var(--color-brand-border)] text-center">
		<div class="w-[72px] h-[72px] mx-auto mb-[20px] rounded-[20px] border border-[#E1EAEE] bg-[#F5F6F9] shadow-sm flex items-center justify-center">
		...
		<h2 class="font-montserrat text-[1.25rem] font-[800] text-[var(--color-brand-text)] mb-[10px]">Nessun indirizzo salvato</h2>
		...
		<button
			@click="emit('create')"
			class="btn-cta btn-compact inline-flex items-center gap-[6px]">
	...
	<div v-else class="space-y-[10px]">
		<div
			...
			:key="address.id"
			:class="['bg-white rounded-[20px] p-[16px] desktop:p-[20px] border transition-all', address.default ? 'border-[var(--color-brand-primary)] shadow-sm' : 'border-[var(--color-brand-border)] hover:border-[#D7E1E4]']">
			<div class="flex flex-col gap-[12px] desktop:flex-row desktop:items-start desktop:gap-[16px]">
				...
				<div class="flex-1 min-w-0">
					<div class="flex flex-wrap items-center gap-[8px] mb-[4px]">
						<h3 class="font-montserrat text-[1rem] font-[800] text-[var(--color-brand-text)]">{{ address.name }}</h3>
						<span v-if="address.default" class="inline-block px-[8px] py-[2px] rounded-[20px] text-[0.6875rem] font-medium bg-[var(--color-brand-primary)]/10 text-[var(--color-brand-primary)]">
							Predefinito
						</span>
					</div>
					<p class="text-[0.875rem] text-[var(--color-brand-text)]">{{ address.address }}</p>
				...
				<div class="flex flex-wrap gap-[8px] shrink-0 desktop:max-w-[220px] desktop:justify-end">
					...
					<template v-if="deleteConfirmId !== address.id">
						<button
							...
							class="btn-secondary btn-compact inline-flex items-center gap-[6px] text-red-600 hover:!border-red-200 hover:!bg-red-50 hover:text-red-700">
					...
					<template v-else>
						<div class="flex flex-wrap gap-[8px]">
							<button
								...
								class="btn-danger btn-compact inline-flex items-center justify-center gap-[4px] px-[14px] !py-[8px] text-[0.75rem] disabled:opacity-60">
							...
							<button
								...
								class="btn-secondary btn-compact inline-flex items-center justify-center gap-[4px] px-[14px] !py-[8px] text-[0.75rem]">
