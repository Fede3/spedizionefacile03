...
<template>
	<Teleport to="body">
		<div v-if="show" class="fixed inset-0 z-[9999] flex items-center justify-center">
			...
			<div ref="dialogRef" role="dialog" aria-modal="true" aria-labelledby="cancel-modal-title" class="relative bg-white rounded-[14px] shadow-lg max-w-[520px] w-full mx-[16px] p-[32px] z-[1]">
				<!-- Header -->
				...
				<template v-else-if="refundEligibility">
					...
					<!-- Eligible -->
					<template v-else>
						<div class="bg-[#F8F9FB] rounded-[14px] p-[16px] mb-[16px]">
						...
						<div class="mb-[16px]">
							...
							<textarea :value="cancelReason" @input="emit('update:cancelReason', $event.target.value)"
								...
								class="w-full bg-[#F8F9FB] border border-[var(--color-brand-border)] rounded-[14px] p-[10px] text-[0.875rem] resize-none"></textarea>
						</div>
