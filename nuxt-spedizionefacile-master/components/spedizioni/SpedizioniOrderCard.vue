	...
	<div class="overflow-hidden rounded-[12px] border border-[#E9EBEC] bg-white shadow-[0_10px_32px_rgba(15,23,42,0.05)]">
		...
		<div class="border-b border-[#E9EBEC] bg-[linear-gradient(180deg,#FBFCFD_0%,#F5F8FA_100%)] px-[16px] py-[14px] tablet:px-[20px]">
			<div class="flex flex-col gap-[10px] desktop:flex-row desktop:items-center desktop:justify-between">
				<div class="flex flex-wrap items-center gap-[10px]">
					<span class="rounded-[12px] bg-[#095866] px-[10px] py-[4px] text-[0.75rem] font-mono font-bold text-white">SF-{{ String(order.id).padStart(6, '0') }}</span>
					<div class="min-w-0">
						<p class="text-[1rem] font-bold leading-[1.1] text-[#252B42]">{{ getOrderPackageLabel(order) }}</p>
				...
				<div class="flex flex-wrap items-center gap-[8px]">
					...
					<span class="inline-flex items-center rounded-full bg-[#F2F6F8] px-[12px] py-[5px] text-[0.75rem] font-semibold text-[#095866]">{{ getOrderSubtotalLabel(order) }}</span>
				</div>
			</div>
		</div>
		...
		<div class="px-[16px] py-[16px] tablet:px-[20px]">
			<div class="space-y-[12px]">
				...
				<div class="flex flex-wrap items-center gap-[8px]">
					<span class="inline-flex items-center rounded-full bg-[#F4F7F9] px-[11px] py-[6px] text-[0.75rem] font-semibold text-[#095866]">{{ getOrderPackageLabel(order) }}</span>
					<span class="inline-flex items-center rounded-full bg-[#FFF5EB] px-[11px] py-[6px] text-[0.75rem] font-semibold text-[#E44203]">{{ getServiceLabel(order) }}</span>
				</div>
			</div>
		</div>
