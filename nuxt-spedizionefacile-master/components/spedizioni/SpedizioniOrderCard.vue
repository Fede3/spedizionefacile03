...
<template>
	<div class="overflow-hidden rounded-[12px] border border-[var(--color-brand-border)] bg-white shadow-[0_10px_32px_rgba(15,23,42,0.05)]">
		<!-- Header -->
		<div class="border-b border-[var(--color-brand-border)] bg-[linear-gradient(180deg,#FBFCFD_0%,#F5F8FA_100%)] px-[16px] py-[14px] tablet:px-[20px]">
			<div class="flex flex-col gap-[10px] desktop:flex-row desktop:items-center desktop:justify-between">
				<div class="flex flex-wrap items-center gap-[10px]">
					<span class="rounded-[12px] bg-[var(--color-brand-primary)] px-[10px] py-[4px] text-[0.75rem] font-mono font-bold text-white">SF-{{ String(order.id).padStart(6, '0') }}</span>
					<div class="min-w-0">
						<p class="text-[1rem] font-bold leading-[1.1] text-[var(--color-brand-text)]">{{ getOrderPackageLabel(order) }}</p>
						<p class="mt-[2px] text-[0.8125rem] text-[#6F7B88]">BRT {{ getServiceLabel(order) }} · {{ getOrderDateLabel(order) }}</p>
					</div>
				</div>
			...
			<div class="flex flex-col gap-[10px] desktop:flex-row desktop:items-center desktop:justify-between">
				<div class="flex flex-col gap-[8px] tablet:flex-row tablet:flex-wrap tablet:items-center">
					<NuxtLink v-if="isPendingPayment(order)" :to="`/checkout?order_id=${order.id}`"
						class="inline-flex w-full items-center justify-center gap-[6px] px-[16px] py-[8px] bg-[var(--color-brand-accent)] text-white rounded-[50px] text-[0.8125rem] font-semibold hover:bg-[var(--color-brand-accent-hover)] transition-all tablet:w-auto">
					...
					<button v-if="!isAlreadySaved(order)" type="button" @click="emit('saveToConfigured', order)"
						...
						class="inline-flex w-full items-center justify-center gap-[6px] rounded-[12px] bg-[var(--color-brand-primary)] px-[14px] py-[9px] text-[0.8125rem] font-semibold text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60 cursor-pointer tablet:w-auto">
				...
				<NuxtLink :to="`/account/spedizioni/${order.id}`" title="Vedi dettagli"
					class="inline-flex w-full items-center justify-center gap-[8px] rounded-[12px] bg-[var(--color-brand-primary)]/10 px-[14px] py-[10px] text-[0.875rem] font-semibold text-[var(--color-brand-primary)] transition hover:bg-[var(--color-brand-primary)]/20 desktop:w-auto">
