			...
			<div v-for="pr in requests" :key="pr.id" class="rounded-[16px] border border-[#E9EBEC] bg-white p-[16px] tablet:p-[18px] transition-colors hover:border-[#D0D7DA]">
				<div class="flex flex-col gap-[14px] desktop:flex-row desktop:items-start desktop:justify-between">
					...
					<div v-if="pr.status === 'pending'" class="flex flex-col gap-[8px] shrink-0 tablet:flex-row desktop:min-w-[220px] desktop:justify-end">
						<button @click="emit('approve', pr.id)" :disabled="actionLoading === `pro-${pr.id}`" class="inline-flex min-h-[42px] items-center justify-center gap-[4px] px-[16px] py-[8px] bg-emerald-600 hover:bg-emerald-700 text-white rounded-[10px] text-[0.8125rem] font-medium transition-colors cursor-pointer disabled:opacity-50">
						...
						<button @click="emit('reject', pr.id)" :disabled="actionLoading === `pro-${pr.id}`" class="inline-flex min-h-[42px] items-center justify-center px-[16px] py-[8px] bg-red-50 hover:bg-red-100 text-red-700 rounded-[10px] text-[0.8125rem] font-medium transition-colors cursor-pointer border border-red-200 disabled:opacity-50">
							Rifiuta
						</button>
					</div>
				</div>
