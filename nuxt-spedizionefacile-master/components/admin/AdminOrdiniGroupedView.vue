...
<template>
	<div class="space-y-[12px]">
		<div v-for="group in groupedOrders" :key="group.user?.id || 'unknown'" class="border border-[#E9EBEC] rounded-[12px] overflow-hidden">
			<!-- Header utente -->
			<button type="button" @click="emit('toggle-user', group.user?.id)" class="w-full bg-[#F5F6F9] px-[14px] py-[14px] tablet:px-[20px] flex flex-col gap-[12px] cursor-pointer hover:bg-[rgba(9,88,102,0.03)] transition-colors tablet:flex-row tablet:items-center tablet:justify-between" :class="expandedUsers.has(group.user?.id) ? 'border-b border-[#E9EBEC]' : ''">
				<div class="flex min-w-0 items-center gap-[12px]">
					...
					<div class="min-w-0 text-left">
						<div class="flex flex-wrap items-center gap-[8px]">
							...
							<span :class="['px-[8px] py-[2px] rounded-full text-[0.625rem] font-semibold uppercase tracking-[0.5px]', (group.user?.user_type || 'privato') === 'commerciante' ? 'bg-[#095866]/10 text-[#095866]' : 'bg-gray-100 text-gray-600']">
								{{ (group.user?.user_type || 'privato') === 'commerciante' ? 'Commerciante' : 'Privato' }}
							</span>
							<span v-if="group.user?.role === 'Partner Pro'" class="px-[8px] py-[2px] rounded-full text-[0.625rem] font-semibold uppercase tracking-[0.5px] bg-[#E44203]/10 text-[#E44203]">Pro</span>
			...
			<div v-if="expandedUsers.has(group.user?.id)">
				...
				<div class="divide-y divide-[#F0F0F0]">
					<div v-for="order in group.orders" :key="order.id" class="grid gap-[12px] px-[14px] py-[12px] hover:bg-[rgba(9,88,102,0.03)] transition-colors tablet:grid-cols-[minmax(0,1fr)_auto] tablet:items-center tablet:px-[20px]">
						...
						<div class="flex flex-wrap items-center gap-[8px] tablet:justify-end">
							...
							<button type="button" @click="emit('show-detail', order)" class="inline-flex items-center gap-[5px] px-[12px] py-[7px] rounded-[12px] bg-white border border-[#D7E1E4] hover:border-[#095866] hover:text-[#095866] text-[#404040] text-[0.75rem] cursor-pointer font-medium transition-colors">
