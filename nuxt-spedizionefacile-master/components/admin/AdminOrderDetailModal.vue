	...
	<div v-if="order" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-[12px] tablet:p-[20px]" @click.self="emit('close')">
		<div class="bg-white rounded-[12px] p-[18px] tablet:p-[28px] shadow-2xl max-w-[750px] w-full max-h-[85vh] overflow-y-auto">
			<div class="mb-[20px] flex items-center justify-between gap-[12px] border-b border-[#EEF1F3] pb-[16px]">
				<h3 class="text-[1.125rem] font-bold text-[var(--color-brand-text)]">Ordine #{{ order.id }}</h3>
			...
			<div v-if="order.brt_parcel_id" class="bg-indigo-50 rounded-[12px] p-[16px] mb-[16px]">
				...
				<div class="grid grid-cols-2 gap-[8px] text-[0.8125rem]">
					<div><span class="text-[var(--color-brand-text-secondary)]">Parcel ID:</span> <span class="font-mono font-medium text-indigo-800">{{ order.brt_parcel_id }}</span></div>
			...
			<!-- PUDO section -->
			<div class="bg-[var(--color-brand-primary)]/10 rounded-[12px] p-[16px] mb-[16px]">
				<div class="flex items-center justify-between mb-[6px]">
					...
					<div class="flex gap-[6px]">
						<button @click="emit('toggle-pudo-selector')" class="px-[10px] py-[4px] rounded-[12px] bg-[var(--color-brand-primary)] text-white text-[0.75rem] font-medium cursor-pointer hover:bg-[var(--color-brand-primary-hover)] transition-colors inline-flex items-center gap-[4px]">
			...
			<div v-if="order.packages?.length" class="mb-[16px]">
				...
				<div class="space-y-[6px]">
					<div v-for="pkg in order.packages" :key="pkg.id" class="p-[12px] rounded-[12px] border border-[var(--color-brand-border)] bg-[#FAFBFC] text-[0.8125rem]">
			...
			<div v-if="order.transactions?.length">
				...
				<div class="space-y-[6px]">
					<div v-for="tx in order.transactions" :key="tx.id" class="flex items-center justify-between p-[12px] rounded-[12px] border border-[var(--color-brand-border)] bg-[#FAFBFC] text-[0.8125rem]">
						<div>
