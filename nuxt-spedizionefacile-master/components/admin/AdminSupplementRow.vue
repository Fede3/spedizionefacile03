...
<template>
	<div class="grid grid-cols-1 tablet:grid-cols-[120px_160px_1fr_auto_auto] gap-[8px] items-center p-[12px] rounded-[12px] border border-[var(--color-brand-border)] bg-[#FAFBFC]">
		<label class="text-[0.75rem] text-[#6A7486]">Prefisso CAP
			<input v-model="rule.prefix" type="text" inputmode="numeric" maxlength="5" class="mt-[4px] w-full h-[36px] px-[10px] rounded-[12px] border border-[var(--color-brand-border)] bg-white text-[0.8125rem]">
		</label>
		<label class="text-[0.75rem] text-[#6A7486]">Importo (&euro;)
			<input :value="supplementAmountToEuro(rule)" @input="updateSupplementAmountFromEuro(rule, $event.target.value)" type="text" class="mt-[4px] w-full h-[36px] px-[10px] rounded-[12px] border border-[var(--color-brand-border)] bg-white text-[0.8125rem]">
		</label>
		<label class="text-[0.75rem] text-[#6A7486]">Applica a
			<select v-model="rule.apply_to" class="mt-[4px] w-full h-[36px] px-[10px] rounded-[12px] border border-[var(--color-brand-border)] bg-white text-[0.8125rem]">
