...
<template>
	...
	<template v-else>
		...
		<div class="desktop:hidden grid grid-cols-1 tablet:grid-cols-2 gap-[12px]">
			<div v-for="u in users" :key="u.id" class="rounded-[12px] border-[1.5px] border-[#DFE2E7] bg-white p-[14px] tablet:p-[16px] shadow-sm">
				<div class="flex items-start gap-[12px]">
					<div class="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[14px] bg-[#EDF5F6] text-[var(--color-brand-primary)]">
					...
					<div class="min-w-0 flex-1">
						<div class="flex flex-wrap items-center gap-[8px]">
							...
							<span :class="u.email_verified_at ? 'text-[#0a8a7a] bg-[#f0fdf4]' : 'text-amber-700 bg-amber-50'" class="inline-flex items-center gap-[4px] px-[8px] py-[3px] rounded-full text-[0.6875rem] font-medium">
								{{ u.email_verified_at ? 'Verificato' : 'Non verificato' }}
							</span>
						</div>
						<p class="mt-[4px] text-[0.8125rem] text-[var(--color-brand-text-secondary)] break-all">{{ u.email }}</p>
					</div>
				</div>

				<div class="mt-[12px] grid grid-cols-1 gap-[10px] text-[0.8125rem] tablet:grid-cols-2">
					<div class="rounded-[14px] bg-[#F8FAFB] px-[12px] py-[10px]">
						<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Telefono</p>
						<p class="mt-[4px] text-[var(--color-brand-text)]">{{ u.telephone_number || '\u2014' }}</p>
					</div>
					<div class="rounded-[14px] bg-[#F8FAFB] px-[12px] py-[10px]">
						<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Registrazione</p>
						<p class="mt-[4px] text-[var(--color-brand-text)]">{{ formatDate(u.created_at) }}</p>
					</div>
					<div class="rounded-[14px] bg-[#F8FAFB] px-[12px] py-[10px]">
						<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Ruolo attuale</p>
						<div class="mt-[4px]">
							<span :class="['px-[8px] py-[2px] rounded-full text-[0.625rem] font-semibold uppercase tracking-[0.5px]', (u.role || 'User') === 'Partner Pro' ? 'bg-[#eef7f8] text-[#095866]' : (u.role || 'User') === 'Admin' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600']">
								{{ (u.role || 'User') === 'Partner Pro' ? 'Partner Pro' : (u.role || 'User') === 'Admin' ? 'Admin' : 'Cliente' }}
							</span>
						</div>
					</div>
					<div class="rounded-[14px] bg-[#F8FAFB] px-[12px] py-[10px]">
						<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Referral</p>
				...
				<div class="mt-[12px] grid grid-cols-1 gap-[8px]">
					<select :value="u.role || 'User'" @change="emit('role-change', u, $event.target.value); $event.target.value = u.role || 'User'" :disabled="actionLoading === `role-${u.id}`" class="w-full px-[12px] py-[10px] rounded-[14px] bg-white border border-[#E0E6E8] text-[0.875rem] cursor-pointer focus:border-[var(--color-brand-primary)] focus:outline-none">
						<option value="User">Cliente</option>
						<option value="Partner Pro">Partner Pro</option>
						<option value="Admin">Admin</option>
					</select>
					<div class="grid grid-cols-2 gap-[8px]">
						<button v-if="!u.email_verified_at" @click="emit('approve', u.id)" :disabled="actionLoading === u.id" class="inline-flex min-h-[42px] items-center justify-center gap-[4px] px-[12px] py-[9px] rounded-[14px] bg-[var(--color-brand-primary)] text-white text-[0.8125rem] font-medium cursor-pointer disabled:opacity-60">
							Approva
						</button>
						<div v-else class="inline-flex min-h-[42px] items-center justify-center px-[12px] py-[9px] rounded-[14px] bg-[#f0fdf4] text-[#0a8a7a] text-[0.8125rem] font-medium">
							Verificato
						</div>
						<button @click="emit('delete', u)" :disabled="actionLoading === u.id" class="inline-flex min-h-[42px] items-center justify-center gap-[4px] px-[12px] py-[9px] rounded-[14px] bg-red-600 text-white text-[0.8125rem] font-medium cursor-pointer disabled:opacity-60">
							Elimina
						</button>
					</div>
				</div>
			...
			<table class="w-full text-[0.875rem]">
				...
				<tbody>
					<tr v-for="(u, idx) in users" :key="u.id" :class="['border-b border-[#F0F0F0] last:border-0', idx % 2 === 1 ? 'bg-[#FAFBFC]' : '']">
						...
						<td class="py-[14px]">
							<span :class="u.email_verified_at ? 'text-[#0a8a7a] bg-[#f0fdf4]' : 'text-amber-700 bg-amber-50'" class="inline-flex items-center gap-[4px] px-[8px] py-[2px] rounded-full text-[0.6875rem] font-medium">
						...
						<td class="py-[14px] text-right">
							<div class="flex justify-end gap-[6px]">
								<button v-if="!u.email_verified_at" @click="emit('approve', u.id)" :disabled="actionLoading === u.id" class="px-[10px] py-[6px] rounded-[14px] bg-[var(--color-brand-primary)] text-white text-[0.75rem] cursor-pointer disabled:opacity-60 flex items-center gap-[4px]">
								...
								<button @click="emit('delete', u)" :disabled="actionLoading === u.id" class="px-[10px] py-[6px] rounded-[14px] bg-red-600 text-white text-[0.75rem] cursor-pointer disabled:opacity-60 flex items-center gap-[4px]">
