...
<template>
	<div class="border border-[var(--color-brand-border)] rounded-[14px] overflow-hidden bg-white">
		<!-- Toolbar -->
		<div v-if="editor" class="flex flex-wrap items-center gap-[2px] px-[8px] py-[6px] bg-[#F5F6F9] border-b border-[var(--color-brand-border)]">
			<!-- Bold -->
			...
			<!-- Italic -->
			<button type="button" class="toolbar-btn" :class="{ active: editor.isActive('italic') }" title="Corsivo" aria-label="Corsivo" @click="editor.chain().focus().toggleItalic().run()">
				<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4v3h2.21l-3.42 8H6v3h8v-3h-2.21l3.42-8H18V4z"/></svg>
			</button>
			<!-- Underline -->
			<button type="button" class="toolbar-btn" :class="{ active: editor.isActive('underline') }" title="Sottolineato" aria-label="Sottolineato" @click="editor.chain().focus().toggleUnderline().run()">
			...
			<!-- Bullet list -->
			<button type="button" class="toolbar-btn" :class="{ active: editor.isActive('bulletList') }" title="Elenco puntato" aria-label="Elenco puntato" @click="editor.chain().focus().toggleBulletList().run()">
			...
			<!-- Ordered list -->
			<button type="button" class="toolbar-btn" :class="{ active: editor.isActive('orderedList') }" title="Elenco numerato" aria-label="Elenco numerato" @click="editor.chain().focus().toggleOrderedList().run()">
			...
			<!-- Link -->
			<button type="button" class="toolbar-btn" :class="{ active: editor.isActive('link') }" title="Inserisci link" aria-label="Inserisci link" @click="editor.isActive('link') ? removeLink() : setLink()">
