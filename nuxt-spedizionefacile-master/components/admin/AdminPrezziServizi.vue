	...
	<div class="space-y-[18px]">
		...
		<div class="space-y-[16px]">
			<div
				...
				class="rounded-[12px] border border-[var(--color-brand-border)] bg-white p-[16px] tablet:p-[18px] desktop:p-[20px] shadow-sm overflow-hidden">
				...
				<div class="mt-[16px] grid grid-cols-1 desktop:grid-cols-2 gap-[14px]">
					<label v-if="entry.rule.pricing_type === 'fixed' || entry.rule.price_cents != null" class="text-[0.75rem] text-[#6A7486]">
						Prezzo / fee (&euro;)
						<input :value="keyedRuleAmountToEuro(entry.rule)" @input="updateKeyedRuleAmountFromEuro(entry.rule, $event.target.value)" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.pricing_type === 'threshold_percentage'" class="text-[0.75rem] text-[#6A7486]">
						Soglia (&euro;)
						<input v-model.number="entry.rule.threshold_amount_eur" type="number" min="0" step="0.01" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.pricing_type === 'threshold_percentage'" class="text-[0.75rem] text-[#6A7486]">
						Minimo fisso (&euro;)
						<input :value="keyedRuleMinFeeToEuro(entry.rule)" @input="updateKeyedRuleMinFeeFromEuro(entry.rule, $event.target.value)" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.pricing_type === 'threshold_percentage'" class="text-[0.75rem] text-[#6A7486]">
						Percentuale (%)
						<input v-model.number="entry.rule.percentage_rate" type="number" min="0" step="0.01" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.max_weight_kg != null" class="text-[0.75rem] text-[#6A7486]">
						Peso massimo (kg)
						<input v-model.number="entry.rule.max_weight_kg" type="number" min="0" step="0.01" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.threshold_cm != null" class="text-[0.75rem] text-[#6A7486]">
						Soglia lato (cm)
						<input v-model.number="entry.rule.threshold_cm" type="number" min="0" step="1" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.longest_side_threshold_cm != null" class="text-[0.75rem] text-[#6A7486]">
						Lato lungo oltre (cm)
						<input v-model.number="entry.rule.longest_side_threshold_cm" type="number" min="0" step="1" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.girth_threshold_cm != null" class="text-[0.75rem] text-[#6A7486]">
						Soglia perimetro secondario (cm)
						<input v-model.number="entry.rule.girth_threshold_cm" type="number" min="0" step="1" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.min_longest_side_cm != null" class="text-[0.75rem] text-[#6A7486]">
						Lunghezza minima (cm)
						<input v-model.number="entry.rule.min_longest_side_cm" type="number" min="0" step="1" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.max_secondary_side_cm != null" class="text-[0.75rem] text-[#6A7486]">
						Lato secondario max (cm)
						<input v-model.number="entry.rule.max_secondary_side_cm" type="number" min="0" step="1" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.875rem] text-[var(--color-brand-text)]">
					</label>
				</div>
					...
					<div class="space-y-[8px]">
						<div v-for="(tier, tierIndex) in entry.rule.tiers" :key="`${entry.key}-tier-${tierIndex}`" class="grid grid-cols-1 tablet:grid-cols-[1fr_1fr_auto] gap-[8px] items-end">
							<label class="text-[0.75rem] text-[#6A7486]">
								Fino a kg
								<input v-model.number="tier.up_to_kg" type="number" min="0" step="0.01" placeholder="senza limite" class="mt-[4px] w-full h-[40px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
							</label>
							<label class="text-[0.75rem] text-[#6A7486]">
								Prezzo (&euro;)
								<input :value="formatEuro(toEuros(tier.price_cents || 0))" @input="tier.price_cents = euroToCents($event.target.value) || 0" type="text" class="mt-[4px] w-full h-[40px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
							</label>
				...
				<div class="mt-[16px] grid grid-cols-1 desktop:grid-cols-2 gap-[14px]" v-if="entry.section === 'automatic_supplements'">
					<label v-if="entry.rule.province_codes?.length || entry.key === 'calabria_sardegna_sicilia' || entry.key === 'brt_point_csi'" class="text-[0.75rem] text-[#6A7486]">
						Province
						<input :value="(entry.rule.province_codes || []).join(', ')" @input="updateArrayField(entry.rule, 'province_codes', $event.target.value, { uppercase: true })" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.country_codes?.length" class="text-[0.75rem] text-[#6A7486]">
						Paesi
						<input :value="(entry.rule.country_codes || []).join(', ')" @input="updateArrayField(entry.rule, 'country_codes', $event.target.value, { uppercase: true })" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.keyword_list?.length" class="text-[0.75rem] text-[#6A7486] desktop:col-span-2">
						Keyword località
						<input :value="(entry.rule.keyword_list || []).join(', ')" @input="updateArrayField(entry.rule, 'keyword_list', $event.target.value)" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.flag_keys?.length" class="text-[0.75rem] text-[#6A7486]">
						Flag chiave
						<input :value="(entry.rule.flag_keys || []).join(', ')" @input="updateArrayField(entry.rule, 'flag_keys', $event.target.value)" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
					</label>
					<label v-if="entry.rule.delivery_modes?.length" class="text-[0.75rem] text-[#6A7486]">
						Delivery mode
						<input :value="(entry.rule.delivery_modes || []).join(', ')" @input="updateArrayField(entry.rule, 'delivery_modes', $event.target.value)" type="text" class="mt-[4px] w-full h-[42px] px-[12px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)]">
					</label>
				</div>

				<label class="block mt-[16px] text-[0.75rem] text-[#6A7486]">
					Nota operativa
					<textarea v-model="entry.rule.note" rows="2" class="mt-[4px] w-full px-[12px] py-[10px] rounded-[12px] border border-[#D5DDE1] bg-white text-[0.8125rem] text-[var(--color-brand-text)] resize-y"></textarea>
				</label>
			</div>
		</div>
	</div>
