...
let LeafletNS: typeof import('leaflet') | null = null
let mapInstance: import('leaflet').Map | null = null
let pointsLayer: import('leaflet').LayerGroup | null = null
let referenceLayer: import('leaflet').LayerGroup | null = null
...
const validPoints = computed<BrtPudoNormalized[]>(() =>
	(props.points || []).filter(
		(p) => Number.isFinite(Number(p.latitude)) && Number.isFinite(Number(p.longitude)),
	),
)

const buildIcon = (selected: boolean) => {
	if (!LeafletNS) return null
...
const buildPopupHtml = (p: BrtPudoNormalized): string => {
	const distance = Number.isFinite(Number(p.distance_meters))
		? `<div style="margin-top:4px;color:${tealDark};font-weight:600">${
				Number(p.distance_meters) >= 1000
					? `${(Number(p.distance_meters) / 1000).toFixed(1)} km`
					: `${Math.round(Number(p.distance_meters))} m`
			}</div>`
		: ''
	return `
...
const renderPoints = () => {
	if (!mapInstance || !LeafletNS) return
...
const renderReference = () => {
	...
	if (!referenceLayer) referenceLayer = LeafletNS.layerGroup().addTo(mapInstance)
...
const fitToContent = () => {
	...
	const items = validPoints.value
	const ref = props.referencePoint
	const coords: [number, number][] = items.map((p) => [Number(p.latitude), Number(p.longitude)])
	if (ref) coords.push([ref.latitude, ref.longitude])
	if (!coords.length) {
	...
	if (coords.length === 1) {
		...
		if (onlyCoordinate) mapInstance.setView(onlyCoordinate, 14)
		return
	}
...
const focusSelected = () => {
	if (!props.selectedKey || !mapInstance) return
...
onMounted(async () => {
	...
	try {
		...
		LeafletNS = (mod.default || mod) as typeof import('leaflet')
...
<template>
	<div class="relative w-full h-full min-h-[400px] rounded-[16px] overflow-hidden border border-[var(--color-brand-border,#E9EBEC)] bg-[#F2F8F9]">
		<div ref="mapEl" class="absolute inset-0" :aria-busy="!ready" aria-label="Mappa punti BRT" role="application"></div>
		<div
			v-if="tileError"
