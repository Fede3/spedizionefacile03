...
<template>
  <div class="bg-white rounded-[12px] overflow-hidden">
    ...
    <div class="hidden desktop:flex items-center gap-[16px] p-[16px_20px]">
      ...
      <span class="text-[0.75rem] text-[var(--color-brand-text-secondary)] bg-[#F0F0F0] px-[8px] py-[3px] rounded-[6px] shrink-0">{{ entry.item.services?.service_type?.split(',')[0]?.trim() || 'BRT' }}</span>
      ...
      <div class="flex items-center gap-[8px] shrink-0">
        ...
        <button type="button" @click="emit('delete', entry.item.id)" class="text-red-500 hover:text-red-700 cursor-pointer" title="Elimina" aria-label="Elimina spedizione">
