      ...
      <div class="space-y-[14px]">
        ...
        <div class="inline-flex rounded-[10px] bg-[#F2F4F5] p-[4px]">
          <button type="button" @click="switchTab('login')"
            ...
            class="px-[14px] py-[8px] rounded-[8px] text-[0.8125rem] font-semibold transition cursor-pointer">Accedi</button>
          <button type="button" @click="switchTab('register')"
            :class="tab === 'register' ? 'bg-white text-[#252B42] shadow-sm' : 'text-[#737373]'"
            class="px-[14px] py-[8px] rounded-[8px] text-[0.8125rem] font-semibold transition cursor-pointer">Registrati</button>
        </div>
        ...
        <p v-if="error" class="text-[0.8125rem] text-red-600 bg-red-50 border border-red-200 rounded-[10px] px-[10px] py-[8px]">{{ error }}</p>
        <p v-if="success" class="text-[0.8125rem] text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-[10px] px-[10px] py-[8px]">{{ success }}</p>
    ...
    <template #footer>
      <div class="flex flex-col-reverse tablet:flex-row justify-end gap-[10px]">
        <button type="button" @click="emit('update:open', false)" :disabled="loading"
          class="inline-flex items-center justify-center gap-[6px] px-[16px] min-h-[42px] rounded-[10px] border border-[#D0D0D0] text-[#737373] hover:bg-[#F7F9FA] transition cursor-pointer disabled:opacity-60">Annulla</button>
