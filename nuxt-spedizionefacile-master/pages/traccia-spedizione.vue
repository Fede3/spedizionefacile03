...
const statusColorClass = computed(() => {
	...
	const map = {
		...
		cancelled: 'bg-gray-100 text-[var(--color-brand-text-secondary)]',
	};
	return map[trackingResult.value.raw_status] || 'bg-gray-100 text-[var(--color-brand-text-secondary)]';
});
		...
		<div class="my-container">
			...
			<div class="bg-white rounded-[12px] p-[24px_20px] desktop:p-[32px] shadow-[0_14px_28px_rgba(20,37,48,0.06)] border border-[#E5EAEC]">
				...
				<div class="flex gap-[12px]">
					<input
						...
						class="flex-1 bg-[#F8F9FB] p-[12px_16px] border border-[var(--color-brand-border)] rounded-[12px] text-[0.9375rem] placeholder:text-[#a0a0a0] focus:border-[var(--color-brand-primary)] focus:shadow-[0_0_0_3px_rgba(9,88,102,0.1)] focus:outline-none"
			...
			<div v-if="trackingResult && trackingResult.found" class="mt-[24px] bg-white rounded-[12px] p-[24px_20px] desktop:p-[32px] shadow-[0_14px_28px_rgba(20,37,48,0.06)] border border-[#E5EAEC]">
				...
				<div class="flex items-center justify-between mb-[20px] flex-wrap gap-[10px]">
					<h2 class="text-[1.25rem] font-bold text-[var(--color-brand-text)]">Spedizione trovata</h2>
				...
				<div class="grid grid-cols-1 desktop:grid-cols-2 gap-[16px] mb-[24px]">
					<div class="bg-[#F8F9FB] rounded-[12px] p-[16px]">
						<p class="text-[0.75rem] text-[var(--color-brand-text-secondary)] uppercase font-medium mb-[4px]">Numero Ordine</p>
						<p class="text-[0.9375rem] font-semibold text-[var(--color-brand-text)]">#{{ trackingResult.order_id }}</p>
					</div>
					<div class="bg-[#F8F9FB] rounded-[12px] p-[16px]">
						<p class="text-[0.75rem] text-[var(--color-brand-text-secondary)] uppercase font-medium mb-[4px]">Data Ordine</p>
						<p class="text-[0.9375rem] font-semibold text-[var(--color-brand-text)]">{{ trackingResult.created_at || '—' }}</p>
					</div>
					<div v-if="trackingResult.brt_parcel_id" class="bg-[#F8F9FB] rounded-[12px] p-[16px]">
						<p class="text-[0.75rem] text-[var(--color-brand-text-secondary)] uppercase font-medium mb-[4px]">Codice BRT</p>
						<p class="text-[0.9375rem] font-semibold text-[var(--color-brand-text)] font-mono">{{ trackingResult.brt_parcel_id }}</p>
					</div>
					<div class="bg-[#F8F9FB] rounded-[12px] p-[16px]">
						<p class="text-[0.75rem] text-[var(--color-brand-text-secondary)] uppercase font-medium mb-[4px]">Stato Attuale</p>
						<p class="text-[0.9375rem] font-semibold text-[var(--color-brand-text)]">{{ trackingResult.status_description }}</p>
					</div>
				</div>

				<!-- Timeline stati -->
				<div v-if="currentStepIndex >= 0" class="mb-[24px]">
					<h3 class="text-[0.875rem] font-semibold text-[var(--color-brand-text)] mb-[16px]">Avanzamento spedizione</h3>
					<div class="flex items-start justify-between relative">
						<!-- Linea di progresso sfondo -->
						<div class="absolute top-[18px] left-[24px] right-[24px] h-[3px] bg-[var(--color-brand-border)] rounded-full"></div>
						<!-- Linea di progresso attiva -->
						<div
							class="absolute top-[18px] left-[24px] h-[3px] bg-[var(--color-brand-primary)] rounded-full transition-[width] duration-500"
						...
						<div v-for="(step, idx) in statusTimeline" :key="step.key" class="flex flex-col items-center relative z-10 flex-1">
							<div
								:class="idx <= currentStepIndex ? 'bg-[var(--color-brand-primary)] text-white' : 'bg-[var(--color-brand-border)] text-[#A0A5AB]'"
							...
							<p :class="idx <= currentStepIndex ? 'text-[var(--color-brand-text)] font-semibold' : 'text-[#A0A5AB]'" class="text-[0.6875rem] mt-[8px] text-center leading-tight">
								{{ step.label }}
							</p>
						</div>
					</div>
				...
				<!-- Link tracking BRT -->
				<div v-if="trackingResult.brt_tracking_url" class="border-t border-[var(--color-brand-border)] pt-[16px]">
					<a
					...
					<p class="text-[0.8125rem] text-[var(--color-brand-text-secondary)] mt-[8px]">Clicca per vedere tutti i dettagli e gli aggiornamenti in tempo reale sul sito ufficiale BRT.</p>
				</div>
			</div>
			...
			<div v-else-if="trackingResult && !trackingResult.found" class="mt-[24px] bg-white rounded-[12px] p-[24px_20px] desktop:p-[32px] shadow-[0_14px_28px_rgba(20,37,48,0.06)] border border-[#E5EAEC]">
				<div class="flex items-start gap-[12px] mb-[16px]">
					...
					<div>
						<p class="text-[0.9375rem] font-semibold text-[var(--color-brand-text)] mb-[4px]">Spedizione non trovata nel nostro sistema</p>
						<p class="text-[0.8125rem] text-[var(--color-brand-text-secondary)]">Il codice inserito non corrisponde a nessuna spedizione nel nostro archivio. Se hai un codice BRT, puoi verificare lo stato direttamente sul sito del corriere.</p>
					</div>
				</div>
				<a
			...
			<p class="text-[#A0A5AB] text-[0.8125rem] text-center mt-[20px]">
				Puoi anche tracciare le tue spedizioni dall'area <NuxtLink to="/account/spedizioni" class="text-[var(--color-brand-primary)] hover:underline">Le tue spedizioni</NuxtLink>.
			</p>
		</div>
	</section>
</template>
