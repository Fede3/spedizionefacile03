...
<template>
	<section v-if="profileUiReady" class="min-h-[600px] py-[40px] desktop:py-[80px]">
		<div class="my-container">
			...
			<!-- Messaggi -->
			<div v-if="messageSuccess" class="mb-[20px] px-[16px] py-[12px] rounded-[50px] text-[0.875rem] font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">{{ messageSuccess }}</div>
			<div v-if="messageError" class="mb-[20px] px-[16px] py-[12px] rounded-[50px] text-[0.875rem] font-medium bg-red-50 text-red-700 border border-red-200">{{ messageError }}</div>
			...
			<div class="rounded-[20px] border border-[#E9EBEC] bg-white p-[20px] tablet:p-[24px] desktop:p-[32px] shadow-sm">
				<div class="flex flex-col gap-[18px]">
					...
					<div class="grid grid-cols-1 tablet:grid-cols-2 gap-[12px]">
						<div v-for="index in 6" :key="`skel-${index}`" class="h-[84px] rounded-[14px] border border-[#EEF1F3] bg-[#FAFBFC] animate-pulse"></div>
					</div>
				</div>
			</div>
		</div>
