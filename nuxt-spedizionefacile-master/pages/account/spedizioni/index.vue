	...
	<section class="min-h-[600px] py-[40px] desktop:py-[80px]">
		<div class="my-container">
			...
			>
				<template #meta>
					<div class="flex flex-wrap gap-[8px]">
						<span class="inline-flex items-center gap-[6px] rounded-full bg-[var(--color-brand-primary)]/10 px-[12px] py-[6px] text-[0.8125rem] font-semibold text-[var(--color-brand-primary)]">{{ orderStats.total }} spedizioni</span>
						<span class="inline-flex items-center gap-[6px] rounded-full bg-[#F0F6F7] px-[12px] py-[6px] text-[0.8125rem] font-semibold text-[var(--color-brand-primary)]">{{ orderStats.open }} aperte</span>
						<span class="inline-flex items-center gap-[6px] rounded-full bg-[#FFF5EB] px-[12px] py-[6px] text-[0.8125rem] font-semibold text-[var(--color-brand-accent)]">{{ orderStats.pending }} da pagare</span>
					</div>
				</template>
			</AccountPageHeader>

			<!-- Filters -->
			<div class="mb-[20px] rounded-[12px] border border-[var(--color-brand-border)] bg-white px-[16px] py-[16px] shadow-sm tablet:px-[20px] tablet:py-[18px]">
				<div class="flex flex-col gap-[14px] desktop:flex-row desktop:items-center desktop:justify-between">
					<div class="max-w-[540px]">
						<p class="text-[0.8125rem] font-semibold uppercase tracking-[0.8px] text-[var(--color-brand-primary)]">Filtri rapidi</p>
						<p class="mt-[4px] text-[0.875rem] leading-[1.5] text-[var(--color-brand-text-secondary)]">Scorri gli stati piu' utili e mantieni il focus sulle spedizioni davvero attive.</p>
					</div>
					<div class="flex w-full flex-col gap-[10px] desktop:w-auto desktop:items-end">
						<div class="flex w-full gap-[8px] overflow-x-auto pb-[2px] -mx-[2px] px-[2px] desktop:w-auto desktop:flex-wrap desktop:overflow-visible desktop:px-0 desktop:mx-0">
							<button v-for="(filter, filterIndex) in filters" :key="filterIndex"
								...
								:class="filterIndex === activeFilter ? 'bg-[var(--color-brand-primary)] text-white shadow-[0_6px_18px_rgba(9,88,102,0.18)]' : 'bg-[#F3F5F6] text-[var(--color-brand-text-secondary)] hover:bg-[#E7ECEE]'"
			...
			<div v-if="ordersStatus === 'pending'" class="space-y-[12px]">
				<div v-for="n in 3" :key="n" class="bg-white rounded-[12px] border border-[var(--color-brand-border)] p-[20px_24px] animate-pulse">
			...
			<!-- Empty state -->
			<div v-else class="bg-white rounded-[12px] p-[48px] border border-[var(--color-brand-border)] shadow-sm text-center">
				...
				<h2 class="text-[1.25rem] font-bold text-[var(--color-brand-text)] mb-[10px]">Nessuna spedizione</h2>
				<p class="text-[var(--color-brand-text-secondary)] text-[0.9375rem] max-w-[400px] mx-auto mb-[24px] leading-[1.6]">Non hai ancora effettuato nessun ordine. Configura la tua prima spedizione per iniziare.</p>
