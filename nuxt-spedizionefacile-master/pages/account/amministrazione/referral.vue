...
const referralStatusStyle = (status) => {
	const palette = {
		...
		paid: { backgroundColor: '#eef7f8', color: '#095866' },
...
<template>
	<section class="min-h-[600px] py-[40px] desktop:py-[60px] desktop-xl:py-[80px]">
		<div class="my-container">
			...
			<div v-if="referralLoading" class="space-y-[16px]">
				<div class="grid grid-cols-1 tablet:grid-cols-3 gap-[16px]">
					<div
						v-for="n in 3"
						:key="`referral-stat-${n}`"
						class="rounded-[20px] border border-[var(--color-brand-border)] bg-white p-[20px] shadow-sm animate-pulse">
						<div class="h-[18px] w-[32%] rounded bg-[#E9EEF2]"></div>
						<div class="mt-[12px] h-[32px] w-[44%] rounded bg-[#E9EEF2]"></div>
					</div>
				</div>
				<div class="rounded-[20px] border border-[var(--color-brand-border)] bg-white p-[24px] shadow-sm animate-pulse">
					<div class="h-[18px] w-[28%] rounded bg-[#E9EEF2]"></div>
					<div class="mt-[18px] space-y-[10px]">
						<div v-for="n in 4" :key="`referral-row-${n}`" class="h-[54px] rounded-[20px] bg-[#F8FAFB]"></div>
					</div>
				</div>
			</div>
			...
			<template v-else-if="referralStats">
				<div class="mb-[20px] rounded-[20px] border border-[var(--color-brand-border)] bg-[#F8FAFB] p-[14px] tablet:p-[18px]">
					<div class="flex flex-col gap-[16px] desktop:flex-row desktop:items-center desktop:justify-between">
						<div>
							<p class="text-[0.75rem] font-semibold uppercase tracking-[0.6px] text-[var(--color-brand-text-secondary)]">Toolbar referral</p>
				...
				<div class="grid grid-cols-1 tablet:grid-cols-3 gap-[16px] mb-[24px]">
					...
					<div class="bg-white rounded-[20px] p-[20px] border border-[var(--color-brand-border)] shadow-sm">
						...
						<p class="text-[1.75rem] font-bold text-[#095866]">&euro;{{ formatCurrency(referralStats.summary?.total_commissions) }}</p>
					</div>
				</div>
					...
					<div v-else class="space-y-[12px]">
						<div class="desktop:hidden grid grid-cols-1 tablet:grid-cols-2 gap-[12px]">
							<div
								...
								:key="usage.id"
								class="rounded-[20px] border border-[var(--color-brand-border)] bg-white p-[14px] shadow-sm">
								...
								<div class="mt-[12px] grid grid-cols-1 gap-[10px] text-[0.8125rem] tablet:grid-cols-2">
									<div class="rounded-[20px] bg-[#F8FAFB] px-[12px] py-[10px]">
										<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Partner Pro</p>
										<p class="mt-[4px] text-[var(--color-brand-text)]">{{ usage.pro_user?.name || '—' }}</p>
									</div>
									<div class="rounded-[20px] bg-[#F8FAFB] px-[12px] py-[10px]">
										<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Acquirente</p>
										<p class="mt-[4px] text-[var(--color-brand-text)]">{{ usage.buyer?.name || '—' }}</p>
									</div>
									<div class="rounded-[20px] bg-[#F8FAFB] px-[12px] py-[10px]">
										<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Ordine</p>
										<p class="mt-[4px] text-[var(--color-brand-text)]">&euro;{{ formatCurrency(usage.order_amount) }}</p>
									</div>
									<div class="rounded-[20px] bg-[#F8FAFB] px-[12px] py-[10px]">
										<p class="text-[0.6875rem] font-semibold uppercase tracking-[0.45px] text-[var(--color-brand-text-muted)]">Commissione</p>
										<p class="mt-[4px] font-semibold text-[#095866]">+&euro;{{ formatCurrency(usage.commission_amount) }}</p>
									</div>
								</div>
								<div class="mt-[10px] flex flex-wrap items-center gap-[8px] text-[0.8125rem]">
									<span class="text-[#095866]">Sconto -&euro;{{ formatCurrency(usage.discount_amount) }}</span>
								</div>
							</div>
						</div>
							...
							<table class="w-full text-[0.875rem]">
								...
								<tbody>
									<tr
										...
										<td class="py-[14px] text-[var(--color-brand-text)]">{{ formatDate(usage.created_at) }}</td>
										<td class="py-[14px]">
										...
										<td class="py-[14px] text-[var(--color-brand-text)]">{{ usage.buyer?.name }}</td>
										<td class="py-[14px] text-right text-[var(--color-brand-text)]">&euro;{{ formatCurrency(usage.order_amount) }}</td>
										<td class="py-[14px] text-right text-[#095866]">-&euro;{{ formatCurrency(usage.discount_amount) }}</td>
										<td class="py-[14px] text-right font-semibold text-[#095866]">
			...
			<div v-else class="rounded-[20px] border border-[var(--color-brand-border)] bg-white p-[24px] text-center text-[var(--color-brand-text-secondary)] shadow-sm">
