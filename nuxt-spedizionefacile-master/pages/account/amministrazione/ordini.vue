...
const activeStatusLabel = computed(() => {
	return orderStatusOptions.find((option) => option.value === ordersStatusFilter.value)?.label || "Tutti gli stati";
});
...
<template>
	<section class="min-h-[600px] py-[40px] desktop:py-[60px] desktop-xl:py-[80px]">
		<div class="my-container">
			...
			<div class="mb-[20px] rounded-[24px] border border-[#E9EBEC] bg-white p-[16px] tablet:p-[20px] desktop:p-[24px] shadow-sm">
				<div class="flex flex-col gap-[16px]">
					<div class="flex flex-col gap-[14px] desktop:flex-row desktop:items-start desktop:justify-between">
						<div class="min-w-0">
							...
							<div class="mt-[6px] flex flex-wrap items-center gap-[8px]">
								...
								<span class="inline-flex items-center rounded-full bg-[#FFF3EC] px-[10px] py-[5px] text-[0.75rem] font-semibold text-[#E44203]">
									{{ formatCents(visibleOrdersTotal) }} €
								</span>
							</div>
						</div>
						<div class="grid grid-cols-1 gap-[8px] tablet:grid-cols-3 desktop:w-auto">
							<button
								type="button"
								@click="fetchOrders"
								class="inline-flex items-center justify-center gap-[6px] rounded-[50px] border border-[#D7E1E4] bg-white px-[14px] py-[10px] text-[0.875rem] font-medium text-[#404040] transition-colors hover:border-[#095866] hover:text-[#095866] cursor-pointer">
							...
							<button
								...
								:class="['inline-flex items-center justify-center gap-[6px] rounded-[50px] px-[14px] py-[10px] text-[0.875rem] font-medium transition-colors cursor-pointer', groupByUser ? 'bg-[#095866] text-white' : 'bg-white border border-[#D7E1E4] text-[#404040] hover:border-[#095866] hover:text-[#095866]']">
							...
							<button
								...
								class="inline-flex items-center justify-center gap-[6px] rounded-[50px] border border-[#D7E1E4] bg-[#F8FAFB] px-[14px] py-[10px] text-[0.875rem] font-medium text-[#404040] transition-colors hover:border-[#095866] hover:text-[#095866] disabled:cursor-not-allowed disabled:opacity-45">
					...
					<div class="grid grid-cols-1 gap-[10px] tablet:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.7fr)]">
						...
						<input v-model="ordersSearch" @input="onOrdersSearch" type="text" placeholder="Cerca per ID, nome, email..." class="w-full pl-[40px] pr-[14px] py-[11px] bg-[#F8F9FB] border border-[#E9EBEC] rounded-[50px] text-[0.875rem] focus:border-[#095866] focus:outline-none" />
					</div>
						<select v-model="ordersStatusFilter" @change="ordersPage = 1; fetchOrders()" class="w-full px-[14px] py-[11px] bg-[#F8F9FB] border border-[#E9EBEC] rounded-[50px] text-[0.875rem] focus:border-[#095866] focus:outline-none cursor-pointer">
			...
			<div class="bg-white rounded-[24px] p-[18px] tablet:p-[24px] desktop:p-[32px] shadow-sm border border-[#E9EBEC]">
				<div class="mb-[20px] flex flex-col gap-[10px] border-b border-[#EEF1F3] pb-[18px] tablet:flex-row tablet:items-end tablet:justify-between">
					...
					<div class="flex flex-wrap gap-[8px]">
						...
						<span v-if="hasActiveFilters" class="inline-flex items-center rounded-full bg-[#FFF3EC] px-[10px] py-[5px] text-[0.75rem] font-medium text-[#E44203]">
							Filtri attivi
						</span>
					</div>
				</div>
				...
				<!-- VISTA RAGGRUPPATA PER UTENTE (collassata) -->
				<div v-else-if="groupByUser" class="space-y-[12px]">
					<div v-for="group in groupedOrders" :key="group.user?.id || 'unknown'" class="border border-[#E9EBEC] rounded-[14px] overflow-hidden">
						...
						<button
							...
							class="w-full bg-[#F8F9FA] px-[14px] py-[14px] tablet:px-[20px] flex flex-col gap-[12px] cursor-pointer hover:bg-[#F0F4F5] transition-colors tablet:flex-row tablet:items-center tablet:justify-between"
							...
							<div class="flex min-w-0 items-center gap-[12px]">
								...
								<div class="min-w-0 text-left">
									...
									<p class="truncate text-[0.75rem] text-[#737373]">{{ group.user?.email }}</p>
								</div>
							</div>
						...
						<div v-if="expandedUsers.has(group.user?.id)">
							...
							<div class="px-[14px] py-[10px] tablet:px-[20px] bg-[#FAFBFC] border-b border-[#F0F0F0] flex flex-col gap-[8px] tablet:flex-row tablet:items-center tablet:gap-[12px]">
								...
								<select
									...
									class="w-full tablet:w-auto px-[10px] py-[6px] rounded-[8px] bg-white border border-[#E9EBEC] text-[0.75rem] cursor-pointer focus:border-[#095866] focus:outline-none">
							...
							<div class="divide-y divide-[#F0F0F0]">
								<div v-for="order in group.orders" :key="order.id" class="grid gap-[12px] px-[14px] py-[12px] hover:bg-[#FAFBFC] transition-colors tablet:grid-cols-[minmax(0,1fr)_auto] tablet:items-center tablet:px-[20px]">
									<div class="flex min-w-0 flex-wrap items-center gap-[10px]">
										...
										<span class="text-[0.75rem] text-[#737373] tablet:hidden">{{ formatDate(order.created_at) }}</span>
									</div>
									<div class="flex flex-wrap items-center gap-[8px] tablet:justify-end">
										...
										<button @click="showOrderDetail(order)" class="inline-flex items-center gap-[5px] px-[12px] py-[7px] rounded-[10px] bg-white border border-[#D7E1E4] hover:border-[#095866] hover:text-[#095866] text-[#404040] text-[0.75rem] cursor-pointer font-medium transition-colors">
										...
										<select @change="changeOrderStatus(order.id, $event.target.value, order.status); $event.target.value = ''" class="px-[10px] py-[7px] rounded-[10px] bg-[#F8FAFB] text-[#252B42] text-[0.75rem] cursor-pointer border border-[#D7E1E4] font-medium focus:border-[#095866] focus:outline-none">
				...
				<!-- VISTA LISTA FLAT (mobile cards + desktop table) -->
				<div v-else class="space-y-[12px]">
					<div class="grid grid-cols-1 gap-[12px] desktop:hidden tablet:grid-cols-2">
						<div v-for="order in ordersData.data" :key="order.id" class="rounded-[16px] border border-[#E9EBEC] bg-[#F8FAFB] p-[14px] shadow-sm">
							...
							<div class="mt-[12px] grid grid-cols-2 gap-[8px]">
								<button @click="showOrderDetail(order)" class="inline-flex items-center justify-center gap-[5px] px-[12px] py-[9px] rounded-[10px] bg-white border border-[#D7E1E4] hover:border-[#095866] hover:text-[#095866] text-[#404040] text-[0.75rem] cursor-pointer font-medium transition-colors">
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-[14px] h-[14px]" fill="currentColor"><path d="M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9M12,17C7,17 2.73,13.89 1,12C2.73,10.11 7,7 12,7C17,7 21.27,10.11 23,12C21.27,13.89 17,17 12,17Z"/></svg>
									Dettagli
								</button>
								<select @change="changeOrderStatus(order.id, $event.target.value, order.status); $event.target.value = ''" class="w-full px-[10px] py-[9px] rounded-[10px] bg-white text-[#252B42] text-[0.75rem] cursor-pointer border border-[#D7E1E4] font-medium focus:border-[#095866] focus:outline-none">
									<option value="" selected disabled>Stato</option>
									<option v-for="s in getAvailableStatuses(order.status)" :key="s.value" :value="s.value">{{ s.label }}</option>
								</select>
							</div>
				...
				<div v-if="ordersData.last_page > 1" class="mt-[22px] flex flex-col gap-[10px] rounded-[18px] border border-[#EEF1F3] bg-[#FAFBFC] px-[14px] py-[12px] tablet:flex-row tablet:items-center tablet:justify-between">
					<p class="text-[0.8125rem] text-[#6A7486]">
						{{ paginationLabel }}
					</p>
			...
			<!-- Order detail modal -->
			<div v-if="selectedOrder" class="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-[12px] tablet:p-[20px]" @click.self="closeOrderDetail">
				<div class="bg-white rounded-[20px] p-[18px] tablet:p-[28px] shadow-2xl max-w-[750px] w-full max-h-[85vh] overflow-y-auto">
					...
					<div v-if="selectedOrder.packages?.length" class="mb-[16px]">
						...
						<div class="space-y-[6px]">
							<div v-for="pkg in selectedOrder.packages" :key="pkg.id" class="p-[12px] rounded-[14px] border border-[#E9EBEC] bg-[#FAFBFC] text-[0.8125rem]">
					...
					<div v-if="selectedOrder.transactions?.length">
						...
						<div class="space-y-[6px]">
							<div v-for="tx in selectedOrder.transactions" :key="tx.id" class="flex items-center justify-between p-[12px] rounded-[14px] border border-[#E9EBEC] bg-[#FAFBFC] text-[0.8125rem]">
								<div>
