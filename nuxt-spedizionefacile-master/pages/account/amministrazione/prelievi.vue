...
const approveWithdrawal = async (id) => {
	...
	try {
		...
		showError(e, "Errore durante l'approvazione.");
	} finally {
...
const confirmReject = async (id) => {
	actionLoading.value = id;
	try {
		await sanctum(`/api/admin/withdrawals/${id}/reject`, { method: 'POST', body: { notes: rejectNotes.value } });
		showSuccess('Prelievo rifiutato.');
		rejectingId.value = null;
		rejectNotes.value = '';
		await fetchWithdrawals();
	} catch (e) {
		showError(e, 'Errore durante il rifiuto.');
	} finally {
		actionLoading.value = null;
	}
};
	...
	<section class="min-h-[600px] py-[40px] desktop:py-[60px] desktop-xl:py-[80px]">
		<div class="my-container">
			...
			<div
				...
				<svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-[18px] h-[18px] shrink-0" fill="currentColor">
				...
				<div v-else class="space-y-[12px]">
					<div
						...
						<div class="flex flex-col desktop:flex-row desktop:items-center justify-between gap-[12px]">
							<div class="flex-1">
								<div class="flex flex-wrap items-center gap-[10px] mb-[6px]">
									...
									<span
										...
										<svg v-else xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-[12px] h-[12px]" fill="currentColor">
											<path
												d="M12,2A10,10 0 0,1 22,12A10,10 0 0,1 12,22A10,10 0 0,1 2,12A10,10 0 0,1 12,2M12,4A8,8 0 0,0 4,12C4,13.85 4.63,15.55 5.68,16.91L16.91,5.68C15.55,4.63 13.85,4 12,4M12,20A8,8 0 0,0 20,12C20,10.15 19.37,8.45 18.32,7.09L7.09,18.32C8.45,19.37 10.15,20 12,20Z" />
										</svg>
							...
							<div
								v-if="w.status === 'pending'"
								class="flex w-full flex-col gap-[8px] tablet:w-auto tablet:flex-row tablet:items-center tablet:justify-end">
