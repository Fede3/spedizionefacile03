...
const {
	...
	actionLoading, actionMessage, formatDate, proRequestStatusConfig,
} = useAdminUtenti();
...
<template>
	<section class="min-h-[600px] py-[32px] tablet:py-[40px] desktop:py-[60px] desktop-xl:py-[80px]">
		<div class="my-container">
			...
			<!-- Sub-tab toggle -->
			<div class="grid grid-cols-1 tablet:grid-cols-2 gap-[4px] mb-[24px] bg-[#F0F0F0] rounded-[12px] p-[4px] w-full tablet:w-fit">
				...
				<div class="mb-[20px] rounded-[12px] border border-[#E9EBEC] bg-[#F8FAFB] p-[14px] tablet:p-[18px] shadow-sm">
					<div class="flex flex-col gap-[14px] desktop:flex-row desktop:items-center desktop:justify-between">
						...
						<div class="flex flex-wrap items-center gap-[8px]">
							<span class="inline-flex min-h-[34px] items-center rounded-full border border-[#DCE7E8] bg-white px-[12px] text-[0.75rem] font-medium text-[#095866]">{{ filteredUsers.length }} risultati</span>
							<button v-if="hasUserFilters" type="button" @click="resetUserFilters" class="inline-flex min-h-[34px] items-center rounded-full border border-[#E9EBEC] bg-white px-[12px] text-[0.75rem] font-medium text-[#737373] transition hover:border-[#D0D7DA] hover:text-[#252B42] cursor-pointer">Azzera filtri</button>
						</div>
					</div>
					<div class="mt-[14px] grid grid-cols-1 gap-[12px] tablet:grid-cols-[minmax(0,1fr)_220px] desktop:grid-cols-[minmax(0,1fr)_240px]">
						<div class="relative min-w-0">
							...
							<input v-model="usersSearch" type="text" placeholder="Cerca per nome, email..." class="w-full pl-[40px] pr-[14px] py-[11px] bg-white border border-[#E9EBEC] rounded-[12px] text-[0.875rem] focus:border-[#095866] focus:outline-none" />
						</div>
						<select v-model="usersRoleFilter" class="w-full px-[14px] py-[11px] bg-white border border-[#E9EBEC] rounded-[12px] text-[0.875rem] focus:border-[#095866] focus:outline-none cursor-pointer">
