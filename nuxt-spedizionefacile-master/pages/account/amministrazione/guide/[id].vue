...
const fetchArticle = async () => {
	...
	} catch (e) { showError(e, "Errore nel caricamento della guida."); }
...
const saveGuide = async () => {
	...
	try {
		...
		showSuccess("Guida aggiornata con successo.");
	...
	<section class="min-h-[600px] py-[40px] desktop:py-[60px] desktop-xl:py-[80px]">
		<div class="my-container max-w-[1400px]">
			...
			<div class="mb-[24px] text-[0.875rem] text-[#737373]">
				...
				<NuxtLink to="/account/amministrazione/guide" class="hover:underline text-[#095866] font-medium">Guide</NuxtLink>
				<span class="mx-[8px] text-[#C8CCD0]">/</span>
				<span class="font-semibold text-[#252B42]">Modifica</span>
			</div>

			<NuxtLink to="/account/amministrazione/guide" class="inline-flex items-center gap-[6px] text-[0.8125rem] text-[#095866] hover:underline font-medium mb-[20px]">
			...
			<h1 class="text-[1.75rem] font-bold text-[#252B42] mb-[24px]">Modifica guida</h1>
			...
			<template v-else>
				<div class="space-y-[20px]">
					...
					<div class="bg-white rounded-[20px] p-[24px] desktop:p-[32px] shadow-sm border border-[#E9EBEC]">
						...
						<div class="space-y-[16px] max-w-[700px]">
							<div>
								...
								<input v-model="form.title" @input="generateSlug" type="text" class="w-full px-[14px] py-[10px] bg-[#F8F9FB] border border-[#E9EBEC] rounded-[10px] text-[0.875rem] focus:border-[#095866] focus:outline-none" placeholder="Titolo della guida" />
							</div>
							<div>
								<label class="block text-[0.8125rem] font-medium text-[#404040] mb-[6px]">Slug (URL)</label>
								<input v-model="form.slug" type="text" class="w-full px-[14px] py-[10px] bg-[#F8F9FB] border border-[#E9EBEC] rounded-[10px] text-[0.875rem] font-mono focus:border-[#095866] focus:outline-none" placeholder="titolo-della-guida" />
							</div>
							<div>
							...
							<div>
								...
								<textarea v-model="form.intro" rows="3" class="w-full px-[14px] py-[10px] bg-[#F8F9FB] border border-[#E9EBEC] rounded-[10px] text-[0.875rem] focus:border-[#095866] focus:outline-none resize-none" placeholder="Paragrafo introduttivo della guida"></textarea>
							</div>
					...
					<div class="bg-white rounded-[20px] p-[24px] desktop:p-[32px] shadow-sm border border-[#E9EBEC]">
						...
						<div class="max-w-[700px]">
							<div v-if="form.image_url" class="mb-[16px]">
								...
								<img :src="form.image_url" alt="Immagine guida" loading="lazy" decoding="async" class="max-w-full max-h-[200px] rounded-[12px] border border-[#E9EBEC] object-cover" />
							</div>
					...
					<div class="flex justify-end">
						<button @click="saveGuide" :disabled="saving" class="px-[24px] py-[12px] bg-[#095866] hover:bg-[#074a56] text-white rounded-[10px] text-[0.875rem] font-medium transition-colors cursor-pointer disabled:opacity-50 flex items-center gap-[8px]">
