...
function saveToSession(state: ShipmentFlowStoreState): void {
...
export const useShipmentFlowStore = defineStore('shipmentFlow', () => {
	...
	const stepNumber = ref<number>(1) // Step corrente del processo (1-5)
	...
	const hasPersistedHydration = ref<boolean>(false)
	...
	const isQuoteStarted = ref<boolean>(false) // true dopo il primo calcolo prezzo

	const totalPrice = ref<number>(0) // Prezzo totale in euro (somma di tutti i pacchi)
	...
	const contentDescription = ref<string>('')
	...
	function applyPersistedState(saved: Partial<ShipmentFlowStoreState> | null): void {
