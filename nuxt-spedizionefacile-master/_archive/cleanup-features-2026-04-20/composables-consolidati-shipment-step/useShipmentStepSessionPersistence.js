...
export const buildPickupRequestPayload = ({ shipmentFlowStore, services } = {}) => {
	const rawPickupDate = String(services?.value?.date || shipmentFlowStore?.pickupDate || shipmentFlowStore?.serviceData?.pickup_request?.date || '').trim();

	const normalizedPickupDate = normalizePickupRequestDate(rawPickupDate);
	const pickupTimeSlot =
		String(services?.value?.time || shipmentFlowStore?.serviceData?.pickup_request?.time_slot || DEFAULT_PICKUP_TIME_SLOT).trim() ||
