/** Sub-composable: stato + API per il calcolo tariffa del preventivo. */
export const usePreventivoCalc = (deps) => {
	...
	const syncQuoteStateFromSession = (sessionData = {}, options = {}) => {
		...
		if (sourceSignature) {
			...
			// Sincronizza anche packages: la session backend ha i campi completi
			// (weight/first_size/second_size/third_size), quelli locali del form
			// preventivo hanno solo quantity+package_type, e senza questo sync
			// persistServicesStep fallisce con "Devi aggiungere almeno 1 collo".
			const sessionPackages = Array.isArray(sessionData?.packages)
				? sessionData.packages.map((pack) => ({ ...pack }))
				: null;
			if (sessionPackages && sessionPackages.length > 0) {
				shipmentFlowStore.packages.splice(0, shipmentFlowStore.packages.length, ...sessionPackages);
			}
	...
	const calculateRate = async ({ silent = false, payload = null } = {}) => {
		...
		const runRequest = async () => {
			...
			try {
				...
				if (!silent) {
					messageError.value = null;
					// GA4 e-commerce: view_item quando il preventivo e' stato calcolato.
					// Trackiamo solo in modalita' NON silent (autoquote dovrebbe restare silent)
					// e con il primo pacco come proxy del quote corrente.
					try {
						const ecom = useEcommerceAnalytics();
						const firstPack = shipmentFlowStore.packages?.[0];
						const priceCents = Math.round(Number(firstPack?.single_price || 0) * 100);
						const quoteType = isOriginItaly?.value && isDestinationItaly?.value ? 'nazionale' : 'internazionale';
						ecom.viewItem({
							quoteType,
							priceCents,
							weightKg: Number(firstPack?.weight || 0),
							packageType: String(firstPack?.type || firstPack?.package_type || 'pacco'),
						});
					} catch { /* no-op: analytics non blocca */ }
				}
				return true;
			} catch (error) {
				if (!silent) {
