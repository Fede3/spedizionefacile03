...
/**
 * FILE: app/Http/Controllers/InvoiceListController.php
 *
 * SCOPO:
 *   Endpoint dedicato alla pagina frontend "/account/fatture". Restituisce SOLO
 *   gli ordini che hanno un'evidenza di fattura (numero SDI, data invio SDI,
 *   oppure richiesta di fattura nei dati di fatturazione snapshot).
 *
 *   L'obiettivo e' evitare che il frontend usi come fallback l'endpoint generico
 *   /api/orders filtrando a mano: qui la logica di "cos'e' una fattura" vive
 *   lato server, coerente con lo stato SDI del dominio.
 *
 * SICUREZZA:
 *   - Richiede auth:sanctum (rotta protetta in routes/api/invoices.php).
 *   - Filtro utente stretto: WHERE user_id = auth()->user()->id.
 *   - Rate limit 30 richieste/min montato a livello di rotta.
 *
 * OUTPUT:
 *   Collection paginata di oggetti { id, order_id, invoice_number, issued_at,
 *   amount_cents, amount_eur_formatted, sdi_status, sdi_sent_at, download_url }.
 *   Il campo `sdi_status` e' letto dal DB quando presente, altrimenti derivato
 *   runtime dai timestamp (pattern pending/sent/accepted/rejected).
 */
