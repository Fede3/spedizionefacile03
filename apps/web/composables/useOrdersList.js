...
export default function useOrdersList() {
	...
	// statusColor / statusStyle — palette unificata via useStatusBadge composable (P5).
	// Restituisce coppia "bg-[#hex] text-[#hex]" derivata dalla stessa fonte unica.
	const statusColor = (status) => {
		const raw = statusRaw(status);
		const s = useStatusBadgeStyle(raw);
		return `bg-[${s.background}] text-[${s.color}]`;
	};
	// statusStyle — alternativa style-inline preferita per nuovi caller.
	const statusStyle = (status) => useStatusBadgeStyle(statusRaw(status));
	...
	return {
		...
		statusRaw, statusColor, statusStyle,
