...
export function useFunnelNavigation() {
	// Track dei timer pendenti (scroll/focus) per cleanup su unmount,
	// evita callback "zombie" che cercano di fare scroll su DOM smontato.
	const pendingScrollTimers = new Set();
	const trackScrollTimer = (delay, callback) => {
	...
	// Frame delay (~24ms): aspetta un repaint dopo blur prima di procedere
	const FRAME_DELAY_MS = 24;
...
// ============================================================================
// SEZIONE 4 — Validation (ex useFunnelValidation)
// ============================================================================
...
export function useFunnelValidation(options) {
	...
	let focusInvalidPackageTimer = null;
	onScopeDispose(() => {
		if (focusInvalidPackageTimer) {
			window.clearTimeout(focusInvalidPackageTimer);
			focusInvalidPackageTimer = null;
		}
	});
	...
	const focusFirstInvalidPackageField = () => {
		nextTick(() => {
			...
			if (focusInvalidPackageTimer) window.clearTimeout(focusInvalidPackageTimer);
			focusInvalidPackageTimer = window.setTimeout(() => {
				focusInvalidPackageTimer = null;
