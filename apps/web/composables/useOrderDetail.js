...
export default function useOrderDetail(orderId) {
	...
	// statusColor mantiene la firma "classi Tailwind bg/text" per retrocompatibilita',
	// statusStyle restituisce { color, background } per uso inline-style preferito.
	const statusColor = (status) => {
		const s = useStatusBadgeStyle(status);
		return `bg-[${s.background}] text-[${s.color}]`;
	};
	const statusStyle = (status) => useStatusBadgeStyle(status);
