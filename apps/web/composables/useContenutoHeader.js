...
export default function useContenutoHeader() {
	...
	// Polling hero homepage: 2 minuti (battery-friendly).
	// Update admin arrivano via 'homepage-image-updated' event + 'storage' event.
	const HERO_POLL_MS = 120_000;
	...
	onMounted(() => {
		...
		if (route.path === '/') {
			if (homepageHeroEndpointAvailable.value) {
				...
				if (homepageHeroPoll) { clearInterval(homepageHeroPoll); homepageHeroPoll = null; }
				homepageHeroPoll = setInterval(refreshHomepageImage, HERO_POLL_MS);
