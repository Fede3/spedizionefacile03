// utils/shipment.ts — Helper funnel spedizione (state machine, stage, routes, derive).
// Alias legacy `summary` punta al payment route: il vecchio step Conferma e' stato unificato nel Pagamento.
...
export type AddressDraft = {
    name?: string; full_name?: string; address?: string; address_number?: string
    intercom_code?: string; city?: string; postal_code?: string; province?: string
    country?: string; additional_information?: string; telephone_number?: string
    email?: string; type?: string
}
export type PackageDraft = {
    package_type?: string; quantity?: number | string; weight?: number | string
    first_size?: number | string; second_size?: number | string; third_size?: number | string
}
export type ShipmentFlowState = {
    quote_ready: boolean; services_ready: boolean; addresses_ready: boolean
    summary_ready: boolean; last_valid_route: string
}
type ServiceDraft = {
    service_type?: string; date?: string
    serviceData?: Record<string, unknown>; sms_email_notification?: boolean
}
export type ShipmentFlowDeriveInput = {
    packages?: PackageDraft[]; pickup_date?: string; content_description?: string
    origin_address?: AddressDraft | null; destination_address?: AddressDraft | null
    services?: ServiceDraft; service_data?: Record<string, unknown>
    sms_email_notification?: boolean; delivery_mode?: string; selected_pudo?: unknown
    flow_state?: Partial<ShipmentFlowState>; client_submission_id?: string
}
type PendingShipmentDraft = ShipmentFlowDeriveInput & { pudo?: unknown }
export type UserStoreLikeInput = {
    shipmentDetails?: { origin_city?: string; destination_city?: string; date?: string }
    packages?: PackageDraft[]; pendingShipment?: PendingShipmentDraft | null
    deliveryMode?: string; selectedPudo?: unknown; pickupDate?: string
    contentDescription?: string; originAddressData?: AddressDraft | null
...
export type StepAddressState = Required<Pick<AddressDraft, 'address' | 'address_number' | 'intercom_code' | 'country' | 'city' | 'postal_code' | 'province' | 'telephone_number' | 'email' | 'type'>> & {
    full_name: string; additional_information: string
}
export type BuiltPendingShipment = {
    origin_address: AddressDraft; destination_address: AddressDraft
    services: ServiceDraft & { serviceData: Record<string, unknown>; sms_email_notification: boolean }
    packages: PackageDraft[]; content_description: string; delivery_mode: string
    pudo: unknown; sms_email_notification: boolean; client_submission_id?: string
}
export type TrimmableUserStore = {
    pendingShipment?: unknown; originAddressData?: unknown; destinationAddressData?: unknown
    selectedPudo?: unknown; servicesArray?: unknown[]; contentDescription?: string
    pickupDate?: string; smsEmailNotification?: boolean; serviceData?: Record<string, unknown>
    packages?: unknown[]; totalPrice?: number; isQuoteStarted?: boolean; stepNumber?: number
}
...
const EMPTY_FLOW_STATE = (): ShipmentFlowState => ({
    quote_ready: false, services_ready: false, addresses_ready: false,
    summary_ready: false, last_valid_route: SHIPMENT_FLOW_ROUTES.packages,
})
// Helper interno: signature diversa dalla versione esportata in utils/auth (qui forza string | undefined).
const getRouteQueryValue = (value: unknown): string | undefined => {
    const raw = Array.isArray(value) ? value[0] : value;
    return raw === undefined || raw === null ? undefined : String(raw);
};
const normalizeShipmentStep = (value: unknown, fallback = 'colli'): string => String(getRouteQueryValue(value) || '').trim() || fallback;
...
export const buildShipmentFlowLocation = (routeLike: RouteLike = {}, step = 'colli') => ({
    ...
    query: { ...(routeLike?.query || {}), step: normalizeShipmentStep(step) },
    hash: routeLike?.hash,
});
...
export const buildShipmentFlowEditLocation = (editId: unknown, step = 'pagamento') => ({
    path: '/la-tua-spedizione/2',
    query: { step: normalizeShipmentStep(step, 'pagamento'), edit: String(editId) },
});
...
const normalizeServiceTypeList = (serviceType: unknown): string[] => String(serviceType || '').split(',').map((v) => v.trim()).filter(Boolean);
...
const hasPackagesReady = (packages: unknown): boolean => hasPackages(packages) && packages.every((p) => hasCompletePackageDetails(p));
const hasAddressDraft = (address: AddressDraft | null | undefined = {}) => {
    const a = address || {};
    return hasMeaningfulText(a.name || a.full_name) && hasMeaningfulText(a.address)
        && hasMeaningfulText(a.city) && hasMeaningfulText(a.postal_code);
};
...
export const deriveShipmentFlowState = (data: ShipmentFlowDeriveInput = {}): ShipmentFlowState => {
    ...
    return {
        quote_ready: quoteReady, services_ready: servicesReady, addresses_ready: addressesReady,
        summary_ready: summaryReady, last_valid_route: lastValidRoute,
    };
};
/** Se il payload contiene gia' flow_state persistito, lo normalizza usando derive come fallback. */
export const resolveShipmentFlowState = (data: ShipmentFlowDeriveInput = {}): ShipmentFlowState => {
    ...
    return {
        ...
        last_valid_route: hasMeaningfulText(raw.last_valid_route) ? String(raw.last_valid_route) : fallback.last_valid_route,
    };
};
...
/** Fra piu' snapshot di flow-state sceglie quello piu' avanzato. */
...
export const getShipmentFlowStage = (routeLike: RouteLike): ShipmentFlowStage | null => {
    ...
    const stepQuery = normalizeShipmentStep((routeLike?.query || {}).step, '');
...
export const isShipmentFlowResumeException = (routeLike: RouteLike): boolean => {
    ...
    if (path.startsWith('/la-tua-spedizione') && hasPositiveQueryId(query.order_id)
        && ['pagamento', 'conferma', 'payment'].includes(stepQuery)) return true;
    return false;
};
/** Decide se l'utente puo' accedere a una route del funnel dato lo stato corrente. */
...
export const buildPendingShipmentFromSession = (data: ShipmentFlowDeriveInput = {}): BuiltPendingShipment | null => {
    ...
    const smsFlag = Boolean(data?.sms_email_notification ?? data?.services?.sms_email_notification ?? data?.service_data?.sms_email_notification);
    ...
    return {
        ...
        services: {
            ...(data.services || {}),
            serviceData: { ...((data.service_data || data?.services?.serviceData || {})), sms_email_notification: smsFlag },
            sms_email_notification: smsFlag,
        },
...
export const deriveShipmentFlowStateFromUserStore = (shipmentFlowStore: UserStoreLikeInput = {}): ShipmentFlowState => {
    ...
    const addressesReady = servicesReady && hasAddressDraft(originAddress) && hasDestinationReady;
    ...
    const pendingSummaryReady = hasPackagesReady(pendingPackages) && pendingServicesReady
        && hasAddressDraft(pendingShipment?.origin_address || null) && pendingDestinationReady;
