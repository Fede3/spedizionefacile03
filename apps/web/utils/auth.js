...
/**
 * Indica se nella Cookie header è presente almeno un cookie di sessione Laravel.
 * Riconosce sia i cookie *_session (laravel_session, app_session) sia XSRF-TOKEN
 * (presente quando Sanctum ha emesso il CSRF token tramite /sanctum/csrf-cookie).
 * @param {string} cookieHeader
 * @returns {boolean}
 */
export const hasAuthSessionCookie = (cookieHeader) =>
	/(?:^|;\s*)(?:[^=;]+_session|XSRF-TOKEN)=/.test(cookieHeader);
