...
const emit = defineEmits<{
	(e: 'close'): void
	(e: 'pudo-selected', pudo: BrtPudoNormalized): void
}>()

// ── Helpers orari ──
const dayNames: Array<{ key: string; label: string; tokens: string[] }> = [
...
const splitHoursParts = (raw: unknown): string[] => {
	if (!raw) return []
	if (Array.isArray(raw)) return raw.map((s) => String(s || '').trim()).filter(Boolean)
	if (typeof raw === 'object') return Object.entries(raw as Record<string, unknown>).map(([k, v]) => `${k}: ${v}`)
	return String(raw).split(/[\n|;]/g).map((s) => s.trim()).filter(Boolean)
}

const weekHours = computed<Array<{ label: string; value: string }>>(() => {
	...
	return dayNames.map((d) => {
		...
		return { label: d.label, value: found ? found.replace(/^[^:]+:\s*/, '').trim() || found : 'Chiuso' }
	})
})
...
const handleChoose = () => {
	if (props.pudo) emit('pudo-selected', props.pudo)
}
</script>
...
<!-- Stili estratti in assets/css/components/sf-pudo-detail-panel.css (importato da main.css). -->
