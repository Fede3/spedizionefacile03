...
<template>
	<div class="grid grid-cols-1 tablet:grid-cols-[120px_160px_1fr_auto_auto] gap-[8px] items-center p-[12px] rounded-[12px] border-[1.5px] border-[#DFE2E7] bg-[#FAFBFC]">
		...
		<label class="text-[0.75rem] text-[var(--color-brand-text-secondary)]">Importo (&euro;)
			<input :value="supplementAmountToEuro(rule)" type="text" class="mt-[4px] w-full h-[36px] px-[10px] rounded-[12px] border-[1.5px] border-[#DFE2E7] bg-white text-[0.8125rem]" @input="updateSupplementAmountFromEuro(rule, $event.target.value)">
		</label>
		...
		<button type="button" role="switch" :aria-checked="rule.enabled ? 'true' : 'false'" aria-label="Attiva supplemento" :class="rule.enabled ? 'bg-[var(--color-brand-primary)]' : 'bg-[#C8CCD0]'" class="relative inline-flex h-[28px] w-[48px] items-center rounded-full transition-colors cursor-pointer mt-[16px]" @click="rule.enabled = !rule.enabled">
