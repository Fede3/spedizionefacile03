...
useBreadcrumbSchema([{ name: 'Home', url: '/' }, { name: 'Traccia spedizione' }]);
...
const statusTimeline = [
	{ key: 'processing', label: 'Pagamento ricevuto' },
	{ key: 'label_generated', label: 'Etichetta generata' },
...
const STATUS_COLOR = {
	...
	processing: TEAL, label_generated: TEAL, in_transit: TEAL, returned: TEAL,
	completed: GREEN, paid: GREEN, delivered: GREEN,
	out_for_delivery: 'bg-[#dff0f3] text-[#074a56] border border-[#b0d8df]',
	in_giacenza: ORANGE, refunded: ORANGE,
	payment_failed: RED, refused: RED,
	cancelled: GREY,
};

const isAlternateEndState = computed(() => !!trackingResult.value && Object.keys(ALT_END_LABELS).includes(trackingResult.value.raw_status));
const alternateEndLabel = computed(() => trackingResult.value ? (ALT_END_LABELS[trackingResult.value.raw_status] || '') : '');
const statusColorClass = computed(() => trackingResult.value ? (STATUS_COLOR[trackingResult.value.raw_status] || GREY) : '');
...
const currentStepIndex = computed(() => {
	...
	return statusTimeline.findIndex(s => s.key === raw);
});
...
const trackShipment = async () => {
	...
	try {
		...
		trackingResult.value = response.found
			? response
			: { found: false, brt_tracking_url: response.brt_tracking_url, message: response.message };
	} catch {
...
onMounted(() => { if (trackingCode.value) trackShipment(); });
</script>
				...
				<div class="p-[18px] sm:p-[22px]" style="background: var(--gradient-page-surface)">
					...
					<div class="rounded-[16px] p-[12px]" data-surface="grey-inset">
						<div class="flex gap-[10px]">
							...
							<SfButton size="lg" class="shrink-0" :loading="isLoading" :disabled="!trackingCode.trim()" @click="trackShipment">
				...
				<div class="p-[20px] sm:p-[24px]" style="background: var(--gradient-page-surface)">
					...
					<div class="rounded-[16px] p-[14px] sm:p-[16px] mb-[16px]" data-surface="grey-inset">
						<div class="grid grid-cols-1 tablet:grid-cols-2 gap-[10px]">
							<div
v-for="info in [
								{ label: 'Numero Ordine', value: `#${trackingResult.order_id}`, show: true },
								{ label: 'Data Ordine', value: trackingResult.created_at || '—', show: true },
								{ label: 'Codice BRT', value: trackingResult.brt_parcel_id, show: !!trackingResult.brt_parcel_id, mono: true },
...
v-for="hint in [
					...
					<div class="flex flex-wrap justify-center gap-[10px]">
						...
						<SfButton to="/contatti" variant="secondary">
							<template #leading>
								<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20,2H4A2,2 0 0,0 2,4V22L6,18H20A2,2 0 0,0 22,16V4A2,2 0 0,0 20,2M20,16H6L4,18V4H20V16Z" /></svg>
							</template>
