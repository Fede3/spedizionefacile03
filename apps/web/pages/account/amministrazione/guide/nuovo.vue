...
const publishStateLabel = computed(() => (form.value.is_published ? 'Pubblicata' : 'Bozza'));
const publishStateHint = computed(() =>
	form.value.is_published
		? 'La guida sara visibile appena completi il salvataggio.'
		: 'Puoi salvarla ora e rifinire contenuti e FAQ con calma.',
);
...
<template>
	<AccountPageSection spacing="sf-stack-section" padding="py-6 tablet:py-7 desktop:py-7">
		<AccountPageHeader
			...
			title="Nuova guida"
			description="Crea una guida con sezioni e FAQ, salva in bozza o pubblica."
			:crumbs="[
