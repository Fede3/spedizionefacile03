				...
				<div class="grid gap-5">
					...
					<SfCard padding="md">
						...
						<template #header>
							...
							<p class="text-sm text-brand-text-secondary mt-0.5">Naming, URL e testo introduttivo della guida.</p>
						</template>

						<div class="grid grid-cols-1 desktop:grid-cols-2 gap-4">
							<SfFormGroup label="Titolo">
								<SfInput v-model="form.title" placeholder="Titolo della guida" @input="generateSlug" />
							</SfFormGroup>
							<SfFormGroup label="Slug (URL)" hint="Si aggiorna dal titolo, puoi rifinirlo a mano.">
								<SfInput v-model="form.slug" placeholder="titolo-della-guida" />
							</SfFormGroup>
							...
							<div class="desktop:col-span-2">
								<SfFormGroup label="Introduzione">
									<SfTextarea v-model="form.intro" :rows="3" placeholder="Paragrafo introduttivo della guida" />
								</SfFormGroup>
							</div>
						</div>
						...
						<div class="flex flex-col tablet:flex-row tablet:items-center tablet:justify-between gap-3 pt-4 mt-4 border-t border-brand-border">
							<div class="grid gap-1 max-w-[34rem]">
								...
								<p class="text-sm text-brand-text-secondary">{{ form.is_published ? 'La guida e\' visibile nel catalogo pubblico.' : 'La guida e\' una bozza non visibile pubblicamente.' }}</p>
							</div>
							<div class="inline-flex items-center gap-2.5 flex-wrap tablet:justify-end">
								...
								<SfBadge :tone="form.is_published ? 'primary' : 'neutral'" size="sm">
									{{ form.is_published ? 'Pubblicata' : 'Bozza' }}
								</SfBadge>
							</div>
						</div>
					</SfCard>
					...
					<SfCard padding="md">
						...
						<div class="grid gap-3 max-w-[700px]">
							<div v-if="form.image_url">
								<img :src="form.image_url" alt="Immagine guida" loading="lazy" decoding="async" width="320" height="180" class="max-w-full max-h-[180px] rounded-control object-cover border border-brand-border">
							</div>
							<div>
					...
					<div class="flex justify-end">
						<SfButton variant="primary" size="md" :loading="saving" loading-text="Salvataggio..." :disabled="saving" @click="saveGuide">
