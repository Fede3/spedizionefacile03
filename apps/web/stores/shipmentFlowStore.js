...
export const useShipmentFlowStore = defineStore('shipmentFlow', () => {
	...
	/**
	 * Reset COMPLETO dello store + sessionStorage. Da chiamare:
	 *   - dopo pagamento riuscito (cosi' un nuovo preventivo parte pulito)
	 *   - quando l'utente clicca "Nuovo preventivo"
	 *   - su logout
	 */
	function resetFlow() {
		if (debounceTimer) {
		...
		isQuoteStarted.value = false
		totalPrice.value = 0
		packages.value = []
		servicesArray.value = []
		contentDescription.value = ''
		pendingShipment.value = null
		originAddressData.value = null
		destinationAddressData.value = null
		pickupDate.value = ''
		editingCartItemId.value = null
		deliveryMode.value = 'home'
		selectedPudo.value = null
		...
		if (import.meta.client) {
			try {
				...
				// sessionStorage non disponibile: ignoriamo
			}
		}
	}
})
