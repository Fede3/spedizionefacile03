...
export const usePreventivoStore = defineStore('preventivo', () => {
	const messageError = ref<Record<string, string[]> | null>(null)
	...
	const setPending = (promise: Promise<boolean> | null, signature: string, silent: boolean, requestId: number) => {
	...
	const syncQuoteStateFromSession = (
		...
		const packages = Array.isArray(sessionData?.packages)
			? sessionData.packages.map((pack: any) => ({ ...pack }))
			: null
	...
	const continueToNextStep = async (deps: {
		shipmentFlowStore: any
		flushLocationDraftsForSubmit: (formatter: typeof formatResolvedLocation) => Promise<void>
		calculateRate: (opts?: { silent?: boolean, payload?: any }) => Promise<boolean>
		ensurePackagesIdentity: () => void
		ensurePrimaryPackage: () => void
		session: any
		refresh: () => Promise<any>
