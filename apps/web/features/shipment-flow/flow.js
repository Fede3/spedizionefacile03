...
export const useShipmentStepFlow = ({
	...
	const addressReadinessItems = computed(() => {
		...
		const orig = originAddress?.value || {};
		const dest = destinationAddress?.value || {};
		const isPudo = (deliveryMode?.value || shipmentFlowStore?.deliveryMode) === "pudo";

		const isAddressComplete = (a) => Boolean(
			String(a.full_name || "").trim() &&
				String(a.address || "").trim() &&
				String(a.address_number || "").trim() &&
				String(a.postal_code || "").trim() &&
				String(a.city || "").trim() &&
				String(a.province || "").trim() &&
				String(a.telephone_number || "").trim() &&
				String(a.email || "").trim(),
		);

		const originDone = isAddressComplete(orig);
		const destDone = isPudo ? Boolean(shipmentFlowStore?.selectedPudo) : isAddressComplete(dest);
		...
		return [
			...
			{
				key: "origin-address",
				label: "Indirizzo mittente completo",
				done: originDone,
			},
			{
				key: "dest-address",
				label: isPudo ? "Punto BRT selezionato" : "Indirizzo destinatario completo",
				done: destDone,
			},
		];
	});
